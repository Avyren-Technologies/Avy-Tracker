...
export class BatteryOptimizationService {
  ...
  async getOptimalUpdateInterval(
    ...
    try {
      ...
      return Math.min(
        Math.max(interval, this.MIN_UPDATE_INTERVAL),
  ...
  private async getMovementState(
    userId: number,
  ...
  private async updateBatteryState(
    userId: number,
    batteryLevel: number,
    isCharging: boolean,
  ): Promise<void> {
  ...
  private async updateMovementState(
    userId: number,
    speed: number,
  ): Promise<void> {
    const currentTime = Date.now();
    ...
    const state: MovementState = {
      ...
      speed,
      lastStateChange:
        oldState && oldState.isMoving === isMoving
          ? oldState.lastStateChange
          : currentTime,
      consecutiveStationary: isMoving
        ? 0
        : oldState
          ? oldState.consecutiveStationary + 1
          : 1,
    };
  ...
  private adjustForMovement(
    ...
    return Math.min(
      interval * (1 + stationaryMultiplier * 0.5),
      this.MAX_UPDATE_INTERVAL,
    );
  }
    ...
    try {
      ...
      if (result.rows.length > 0) {
        ...
        return {
          ...
          tracking_precision: settings.default_tracking_precision || "medium",
        };
      }
