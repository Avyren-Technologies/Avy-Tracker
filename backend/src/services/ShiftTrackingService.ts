...
export class ShiftTrackingService {
  ...
  private async calculateShiftMetrics(
    shiftId: number,
  ...
  async setShiftTimer(userId: number, durationHours: number): Promise<any> {
    ...
    try {
      ...
      const { table, idColumn, userColumn, startColumn, endColumn } =
  ...
  async processPendingTimers(): Promise<number> {
    ...
    try {
      ...
      console.log(
        `Found ${timerResult.rows.length} pending timers to process at ${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}`,
      );
      ...
      for (const timer of timerResult.rows) {
        try {
          ...
          console.error(
            `Error processing shift for timer ID ${timer.id}:`,
            shiftError,
          );
    ...
    try {
      ...
      for (const timer of timerResult.rows) {
        try {
          ...
          console.log(
            `[${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}] Sent reminder notification for timer ID ${timer.id}, user ${timer.user_id}, shift ${timer.shift_id}`,
          );
          sentCount++;
        } catch (error: any) {
          console.error(
            `Error sending reminder for timer ID ${timer.id}:`,
            error,
          );
