...
export class AttendanceRegularizationService {
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO attendance_regularization_requests (
          ...
          request_type, status, current_approver_role, group_admin_id, created_by
        ...
        [
          ...
          requestData.original_start_time || null,
          requestData.original_end_time || null,
          requestData.requested_start_time,
          requestData.requested_end_time,
          requestData.reason,
    ...
    try {
      ...
      if (action === 'approve') {
        if (approverRole === 'group-admin') {
          newStatus = 'group_admin_approved';
          nextApproverRole = 'management';
          
          // Get management user for this company
          ...
          if (managementResult.rows.length > 0) {
            managementId = managementResult.rows[0].id;
          }
  ...
  private async validateRequestData(data: any): Promise<void> {
    ...
    if (new Date(data.requested_end_time) <= new Date(data.requested_start_time)) {
  ...
  private canApprove(request: RegularizationRequest, approverId: number, approverRole: string): boolean {
    ...
    if (approverRole === 'management') {
      return request.status === 'group_admin_approved' && request.management_id === approverId;
    }
