...
const logRequest = (
  ...
  console.log(
    `[${new Date().toISOString()}] Face Verification API - ${endpoint}`,
    {
      userId: req.user?.id,
...
const logResponse = (
  ...
  console.log(
    `[${new Date().toISOString()}] Face Verification API Response - ${endpoint}`,
    {
      success,
...
router.post(
  ...
  async (req: CustomRequest & RegisterFaceRequest, res: Response) => {
...
router.post(
  ...
  verifyToken,
  validateFaceEncoding,
  async (req: CustomRequest & VerifyFaceRequest, res: Response) => {
    const startTime = Date.now();
    ...
    try {
      ...
      if (
        ...
        (livenessScore < 0 || livenessScore > 1)
      ) {
      ...
      if (isLocked) {
        ...
        return res.status(423).json({
          error:
            "Account temporarily locked due to failed verification attempts",
          code: "ACCOUNT_LOCKED",
        });
      }
...
router.get(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      const status =
        await FaceVerificationService.getFaceRegistrationStatus(userId);

      // Get verification statistics
      const stats = await FaceVerificationService.getVerificationStatistics(
        userId,
        30,
      );
      ...
      const responseData = {
        ...
        statistics: {
          ...
          averageSuccessConfidence:
            parseFloat(stats.avg_success_confidence) || 0,
      ...
      if (
        ...
        console.error(
          "Face verification tables not found. Please run the migration first.",
        );
        return res.status(503).json({
          error:
            "Face verification system is not properly configured. Please contact administrator.",
...
router.delete(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      // Delete face profile
      const success = await FaceVerificationService.deleteFaceProfile(
        userId,
        userId,
      );
...
router.put(
  ...
  verifyToken,
  validateFaceEncoding,
  async (req: CustomRequest & UpdateFaceRequest, res: Response) => {
    const startTime = Date.now();
...
router.get(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      // Get security report
      const report = await FaceVerificationService.getSecurityReport(
        userId,
        days,
      );
    ...
    try {
      ...
      if (!["management", "super-admin"].includes(performerRole)) {
        ...
        return res.status(403).json({
          error:
            "Insufficient permissions. Management or Super Admin role required.",
      ...
      // Unlock user
      const success = await FaceVerificationService.unlockUser(
        targetUserId,
        performedBy,
      );
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      if (!sessionId || !userId || !shiftAction || !status) {
        return res.status(400).json({
          error:
            "Missing required fields: sessionId, userId, shiftAction, status",
        });
      }
