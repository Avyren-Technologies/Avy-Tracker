...
router.post(
  ...
  async (req: CustomRequest, res: Response) => {
    ...
    try {
      ...
      if (
        !groupAdminResult.rows.length ||
        !groupAdminResult.rows[0].company_id
      ) {
      ...
      for (let i = 1; i < parsedRows.length; i++) {
        ...
        try {
          const employee = {
            ...
            can_submit_expenses_anytime:
              row[headers["can_submit_expenses_anytime"]]?.toLowerCase() ===
              "true",
          };
          ...
          if (existingUser.rows.length > 0) {
            ...
            else if (existing.employee_number === employee.employee_number)
              duplicateField = "employee number";
            else if (existing.phone === employee.phone)
...
router.put(
  "/employee-expense-permissions/:userId",
  verifyToken,
  async (req: CustomRequest, res: Response) => {
    const client = await pool.connect();
    try {
