...
router.post("/geofence", authenticateToken, async (req: any, res) => {
  try {
    ...
    if (!coordinates || !coordinates.type || !coordinates.coordinates) {
      return res
        .status(400)
        .json({ error: "Valid GeoJSON coordinates are required" });
    }

    if (coordinates.type !== "Point") {
      return res
        .status(400)
        .json({ error: "Only Point geometry type is supported" });
    }
    ...
    try {
      ...
      if (dbError.code === "42883") {
        return res.status(500).json({
          ...
          details:
            "The ST_GeomFromGeoJSON function is not available. PostGIS extension may not be installed.",
        });
      }
      ...
      if (dbError.routine === "errorMissingColumn") {
        return res.status(500).json({
          ...
          details:
            "One of the columns in the query does not exist in the table",
        });
      }
...
router.delete("/geofence/:id", authenticateToken, async (req: any, res) => {
  try {
    ...
    await pool.query("DELETE FROM company_geofences WHERE id = $1", [id]);
