...
const processDateFilters = (
  ...
  if (isNaN(startDate.getTime())) {
    console.warn(
      `Invalid start date provided: ${startDateParam}, using default`,
    );
...
router.get("/:type", verifyToken, async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    if (employeeId && employeeId !== "undefined" && employeeId !== "null")
      filterOptions.employeeId = employeeId as string;
...
async function getExpenseReportData(
  client: PoolClient,
  adminId: number,
  filters: FilterOptions = {},
) {
  const { startDate, endDate } = processDateFilters(
    filters.startDate,
    filters.endDate,
  );
  ...
  const total = categoryResult.rows.reduce(
  ...
  return {
    ...
    departments: [
      ...new Set(
        employeesResult.rows.map((emp) => emp.department).filter(Boolean),
      ),
    ],
  };
}
...
async function getTaskReportData(
  ...
  const completedTasks = parseInt(
    summaryResult.rows[0]?.completed_tasks || "0",
  );
  ...
  return {
    summary: {
      ...
      avgCompletionTime: parseFloat(
        summaryResult.rows[0]?.avg_completion_time || "0",
      ),
...
async function getTravelReportData(
  client: PoolClient,
  adminId: number,
  filters: FilterOptions = {},
) {
  ...
  const expenseBreakdown = categoryResult.rows.map((row) => ({
    ...
    percentage: parseFloat(
      ((parseFloat(row.amount) / (total || 1)) * 100).toFixed(1),
    ),
  }));
  ...
  return {
    ...
    categories: categoryResult.rows.map((row) => ({
      ...
      amount: parseFloat(row.amount),
      percentage:
        total > 0 ? ((parseFloat(row.amount) / total) * 100).toFixed(1) : "0",
    })),
  };
}

async function getPerformanceReportData(
  client: PoolClient,
  adminId: number,
  filters: FilterOptions = {},
) {
  ...
  return {
    summary: {
      ...
      avgWorkingHours: parseFloat(
        summaryResult.rows[0]?.avg_working_hours || "0",
      ),
      ...
      taskCompletionRate: parseFloat(
        summaryResult.rows[0]?.task_completion_rate || "0",
      ),
      expenseApprovalRate: parseFloat(
        summaryResult.rows[0]?.expense_approval_rate || "0",
      ),
    },
  };
}
...
async function getLeaveReportData(
  ...
  return {
    summary: {
      totalEmployeesOnLeave: parseInt(
        summaryResult.rows[0]?.total_employees_on_leave || "0",
      ),
      totalRequests: parseInt(summaryResult.rows[0]?.total_requests || "0"),
      totalDays: parseFloat(summaryResult.rows[0]?.total_days || "0"),
      approvedRequests: parseInt(
        summaryResult.rows[0]?.approved_requests || "0",
      ),
      pendingRequests: parseInt(summaryResult.rows[0]?.pending_requests || "0"),
      rejectedRequests: parseInt(
        summaryResult.rows[0]?.rejected_requests || "0",
      ),
      approvalRate: parseFloat(summaryResult.rows[0]?.approval_rate || "0"),
    },
  };
