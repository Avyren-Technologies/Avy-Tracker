...
export const shiftTimerController = {
  ...
  async setTimer(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!durationHours || typeof durationHours !== "number") {
        res.status(400).json({
          success: false,
          error: "Duration in hours is required and must be a number",
        });
        return;
      }
      ...
      try {
        ...
        if (error.message === "No active shift found") {
          res.status(400).json({
            success: false,
            error:
              "No active shift found. You must start a shift before setting an auto-end timer.",
          });
          return;
        }
        ...
        res.status(500).json({
          success: false,
          error:
            "Failed to set timer. Please ensure you have an active shift and try again.",
        });
      }
    } catch (error: any) {
  ...
  async cancelTimer(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(
        `Error cancelling shift timer for user ${(req as any).user?.id}:`,
        error,
      );
      res.status(500).json({
        success: false,
  ...
  async getTimer(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(
        `Error getting shift timer for user ${(req as any).user?.id}:`,
        error,
      );
      res.status(500).json({
        success: false,
