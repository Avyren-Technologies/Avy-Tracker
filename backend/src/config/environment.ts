...
class EnvironmentService {
    ...
    return {
      ...
      faceVerification: {
        threshold: parseFloat(
          process.env.FACE_VERIFICATION_THRESHOLD || "0.85",
        ),
        livenessThreshold: parseFloat(
          process.env.FACE_LIVENESS_THRESHOLD || "0.7",
        ),
        maxAttempts: parseInt(
          process.env.MAX_FACE_VERIFICATION_ATTEMPTS || "3",
          10,
        ),
      ...
      geofence: {
        ...
        accuracyThreshold: parseInt(
          process.env.GEOFENCE_ACCURACY_THRESHOLD || "10",
          10,
        ),
      ...
      biometric: {
        encryptionKey:
          ...
          "fallback-32-char-encryption-key!!",
        storageTtl: parseInt(
          process.env.BIOMETRIC_STORAGE_TTL || "2592000",
          10,
        ), // 30 days
      ...
      offline: {
        syncInterval: parseInt(
          process.env.OFFLINE_SYNC_INTERVAL || "300000",
          10,
        ), // 5 minutes
  ...
  private validateConfiguration(): void {
    ...
    if (this.config.nodeEnv === "production") {
      ...
      if (
        this.config.biometric.encryptionKey ===
        "fallback-32-char-encryption-key!!"
      ) {
      ...
      if (
        this.config.features.otpVerification &&
        this.config.sms.provider === "console"
      ) {
        errors.push(
          "SMS provider must be configured for OTP verification in production",
        );
      }
    }
    ...
    if (this.config.biometric.encryptionKey.length !== 32) {
      errors.push(
        "BIOMETRIC_ENCRYPTION_KEY must be exactly 32 characters long",
      );
    }

    // Validate face verification thresholds
    if (
      this.config.faceVerification.threshold < 0 ||
      this.config.faceVerification.threshold > 1
    ) {
      errors.push("FACE_VERIFICATION_THRESHOLD must be between 0 and 1");
    }
    if (
      this.config.faceVerification.livenessThreshold < 0 ||
      this.config.faceVerification.livenessThreshold > 1
    ) {
    ...
    if (errors.length > 0) {
      ...
      if (this.config.nodeEnv === "production") {
        throw new Error(
          "Invalid production configuration. Please fix the errors above.",
        );
      } else {
        console.warn(
          "Configuration warnings detected. Application will continue with fallback values.",
        );
      }
    }
  }
