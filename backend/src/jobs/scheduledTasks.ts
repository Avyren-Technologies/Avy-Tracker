...
const shiftTimerProcessor = new CronJob("*/1 * * * *", async () => {
  ...
  const processWithRetry = async () => {
    try {
      ...
      console.log(
        `[${currentTime}] Processing shift timers (attempt ${attempt + 1}/${maxAttempts})...`,
      );
      ...
      if (endedShifts > 0) {
        console.log(
          `[${currentTime}] Successfully auto-ended ${endedShifts} shifts based on timer settings`,
        );
      } else {
      ...
      console.error(
        `[${currentTime}] Error processing shift timers (attempt ${attempt}/${maxAttempts}):`,
        error,
      );
...
const shiftReminderSender = new CronJob("*/1 * * * *", async () => {
  try {
    ...
    console.log(
      `[${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}] Sending shift timer reminders...`,
    );
    const sentReminders = await shiftService.sendTimerReminders(5);

    if (sentReminders > 0) {
      console.log(
        `[${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}] Successfully sent ${sentReminders} shift ending reminders`,
      );
    }
  } catch (error) {
    console.error(
      `[${new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })}] Error sending shift reminders:`,
      error,
    );
...
export const startScheduledJobs = () => {
  ...
  console.log(
    "Scheduled jobs started: Error Log Cleanup, Shift Timer Processor, Shift Reminder Sender",
  );
};
