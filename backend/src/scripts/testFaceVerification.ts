  ...
  try {
    ...
    try {
      ...
      result.rows.forEach((row, index) => {
        console.log(`    User ${index + 1}: ID=${row.id}, registered=${row.face_registered}, enabled=${row.face_enabled}, profile_active=${row.profile_active}`);
      });
    ...
    try {
      ...
      console.log(`  ✅ Statistics query successful, found ${result.rows[0].total_logs} logs in last 30 days`);
