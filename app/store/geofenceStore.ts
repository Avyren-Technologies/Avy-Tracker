...
const debugTokenStorage = async () => {
  try {
    ...
    const { getTokenDebugInfo, repairTokenIssues } = await import(
      "../utils/tokenDebugger"
    );
    ...
    console.log(
      ...
      asyncAuth ? "Found token" : "No token found",
    );
    ...
    try {
      ...
      console.log(
        ...
        secureAuth ? "Found token" : "No token found",
      );
      ...
      if (
        ...
        (tokenDebugInfo.issues.accessTokenMismatch ||
          ...
          (tokenDebugInfo.issues.asyncAccessMissing &&
            !tokenDebugInfo.issues.secureAccessMissing) ||
          (tokenDebugInfo.issues.secureAccessMissing &&
            !tokenDebugInfo.issues.asyncAccessMissing))
      ) {
        console.log(
          "Detected token storage inconsistencies, attempting repair...",
        );
  ...
  try {
    ...
    if (!refreshToken) {
      try {
        ...
        console.error(
          "Error accessing refresh token from SecureStore:",
          secureError,
        );
      }
    }
    ...
    if (response.status === 200) {
      ...
      try {
        ...
        console.error(
          "SecureStore not available in geofenceStore:",
          secureError,
        );
      }
  ...
  persist(
    (set, get) => ({
        ...
        try {
          ...
          try {
            ...
            const response = await axios.get(
              `${API_URL}/api/group-admin-tracking/geofences`,
              {
                headers: {
            ...
            if (response.status === 200) {
              console.log(
                "Successfully fetched geofences:",
            ...
            if (status === 401) {
              console.log(
                `Received 401 with message: ${errorData.error || "No error message"}`,
              );
              ...
              if (token) {
                ...
                try {
                  ...
                  if (retryResponse.status === 200) {
                    console.log(
                      "Successfully fetched geofences after token refresh:",
          ...
          if (error.response?.status === 401) {
            ...
            set({
              error: error.response?.data?.error || "Failed to fetch geofences",
            });
          }
        } finally {
          set({ isLoading: false });
      ...
      createGeofence: async (
        ...
        try {
          ...
          if (
            ...
            const error =
              "Invalid coordinates format. Must be a GeoJSON Point with [longitude, latitude] array";
          ...
          try {
            ...
            const response = await axios.post(
              `${API_URL}/api/group-admin-tracking/geofence`,
              {
                name,
                coordinates: pointCoordinates,
                radius,
            ...
            if (requestError.response?.status === 400) {
              const errorMessage =
                requestError.response.data?.error || "Invalid geofence data";
            ...
            if (requestError.response?.status === 500) {
              ...
              const errorMessage =
                errorData?.message ||
                errorData?.error ||
                "Server error creating geofence";
        ...
        try {
          ...
          try {
            ...
            const response = await axios.put(
              `${API_URL}/api/group-admin-tracking/geofence/${id}`,
              processedData,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
              ...
              if (updatedGeofence.coordinates) {
                ...
                if (typeof updatedGeofence.coordinates === "string") {
                  try {
                    ...
                    console.log(
                      "Received WKB format coordinates, refreshing data...",
                    );
                ...
                if (
                  ...
                  const [longitude, latitude] =
                    updatedGeofence.coordinates.coordinates;
            ...
            if (requestError.response?.status === 500) {
              ...
              const errorMessage =
                errorData?.message ||
                errorData?.error ||
                "Server error updating geofence";
              const errorDetails = errorData?.details || errorData?.hint || "";
      ...
      deleteGeofence: async (id: number) => {
        try {
          ...
          if (error.response?.status === 401) {
            ...
            set({
              error: error.response?.data?.error || "Failed to delete geofence",
            });
          }
        } finally {
          set({ isLoading: false });
      ...
      startEditing: (geofence: Geofence) => {
        ...
        console.log(
          "Starting editing geofence with radius:",
          radius,
          typeof radius,
        );
        ...
        set({
          ...
          editRadius: radius,
          editType:
            geofence.coordinates.type === "Point" ? "circle" : "polygon",
        });
      },
        ...
        const newCoordinates: GeoCoordinates =
          type === "circle"
            ? {
                type: "Point",
                coordinates:
                  get().editCoordinates.type === "Point"
            ...
            : {
                type: "Polygon",
                coordinates:
                  get().editCoordinates.type === "Polygon"
                    ? (get().editCoordinates.coordinates as number[][])
                    : [[]],
              };
      ...
      updateEditCoordinates: (coordinates: GeoCoordinates) =>
        set({ editCoordinates: coordinates }),
