...
export const getTokenDebugInfo = async () => {
  try {
    ...
    // Decode tokens
    const asyncAccessDecoded = asyncAccessToken
      ? decodeToken(asyncAccessToken)
      : null;
    const asyncRefreshDecoded = asyncRefreshToken
      ? decodeToken(asyncRefreshToken)
      : null;
    const secureAccessDecoded = secureAccessToken
      ? decodeToken(secureAccessToken)
      : null;
    const secureRefreshDecoded = secureRefreshToken
      ? decodeToken(secureRefreshToken)
      : null;

    // Check for mismatches
    const accessTokenMismatch =
      asyncAccessToken !== secureAccessToken &&
      asyncAccessToken &&
      secureAccessToken;
    const refreshTokenMismatch =
    ...
    const secureAccessMissing =
      !!asyncAccessToken && !secureAccessToken && Platform.OS !== "web";
    const asyncRefreshMissing = !asyncRefreshToken && !!secureRefreshToken;
    const secureRefreshMissing =
      !!asyncRefreshToken && !secureRefreshToken && Platform.OS !== "web";

    // Check if any token is expired
    const asyncAccessExpired = asyncAccessToken
      ? isTokenExpired(asyncAccessToken)
      : false;
    const secureAccessExpired = secureAccessToken
      ? isTokenExpired(secureAccessToken)
      : false;
    const asyncRefreshExpired = asyncRefreshToken
      ? isTokenExpired(asyncRefreshToken)
      : false;
    const secureRefreshExpired = secureRefreshToken
      ? isTokenExpired(secureRefreshToken)
      : false;
...
export const showTokenDebugAlert = async () => {
  ...
  message += "\nISSUES:\n";
  const issuesList = [];
  if (issues.accessTokenMismatch)
    issuesList.push("- Access tokens mismatch between storages");
  if (issues.refreshTokenMismatch)
    issuesList.push("- Refresh tokens mismatch between storages");
  if (issues.asyncAccessMissing)
    issuesList.push("- AsyncStorage access token missing");
  if (issues.asyncRefreshMissing)
    issuesList.push("- AsyncStorage refresh token missing");
  if (issues.secureAccessMissing)
    issuesList.push("- SecureStore access token missing");
  if (issues.secureRefreshMissing)
    issuesList.push("- SecureStore refresh token missing");
  ...
  Alert.alert("Token Debug", message, [
    { text: "OK" },
    {
      ...
      onPress: () => {
        ...
        Alert.alert(
          "Not Implemented",
          "Copy to clipboard not implemented in this version",
        );
      },
    },
  ]);
  ...
  try {
    ...
    if (asyncStorage.accessToken && !secureStore?.accessToken) {
      ...
      await SecureStore.setItemAsync(
        AUTH_TOKEN_KEY,
        asyncStorage.accessToken.token,
      );
      ...
      await AsyncStorage.setItem(AUTH_TOKEN_KEY, secureStore.accessToken.token);
      console.log(
        "Synchronized access tokens using SecureStore as source of truth",
      );
    }
    ...
    if (asyncStorage.refreshToken && !secureStore?.refreshToken) {
      ...
      await SecureStore.setItemAsync(
        REFRESH_TOKEN_KEY,
        asyncStorage.refreshToken.token,
      );
      ...
      console.log(
        "Synchronized refresh tokens using SecureStore as source of truth",
      );
    }
...
export const logStorageState = async () => {
  try {
    ...
    if (debugInfo) {
      ...
      console.log(
        "- Access token:",
        debugInfo.asyncStorage.accessToken
          ? debugInfo.asyncStorage.accessToken.expired
            ? "EXPIRED"
            : "VALID"
          : "NOT FOUND",
      );
      console.log(
        "- Refresh token:",
        debugInfo.asyncStorage.refreshToken
          ? debugInfo.asyncStorage.refreshToken.expired
            ? "EXPIRED"
            : "VALID"
          : "NOT FOUND",
      );
      ...
      if (Platform.OS !== "web") {
        ...
        console.log(
          "- Access token:",
          debugInfo.secureStore?.accessToken
            ? debugInfo.secureStore.accessToken.expired
              ? "EXPIRED"
              : "VALID"
            : "NOT FOUND",
        );
        console.log(
          "- Refresh token:",
          debugInfo.secureStore?.refreshToken
            ? debugInfo.secureStore.refreshToken.expired
              ? "EXPIRED"
              : "VALID"
            : "NOT FOUND",
        );
      ...
      console.log(
        "\nDetected Issues:",
        issuesList.length ? issuesList.join(", ") : "None",
      );
    } else {
