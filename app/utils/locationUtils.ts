 ...
 */
export function createEnhancedLocation(
  location: LocationObject | any,
): EnhancedLocation {
  if (!location) {
    console.warn(
      "Attempted to create enhanced location from null/undefined value",
    );
    return {
      coords: {
        latitude: 0,
  ...
  const batteryLevel =
    typeof location.batteryLevel === "number"
      ? location.batteryLevel
      : undefined;
  const isMoving =
    typeof location.isMoving === "boolean" ? location.isMoving : undefined;
...
export function getLatitude(
  location: EnhancedLocation | LocationObject | any,
): number {
...
export function getAccuracy(
  ...
  return (
    location.accuracy || (location.coords && location.coords.accuracy) || null
  );
}
