import { CapturedPhoto, FaceDetectionData, LandmarkPoint } from '../types/faceDetection';
...
export class AntiSpoofingService {
   ...
   */
  static async analyzeImage(photo: CapturedPhoto, faceData: FaceDetectionData): Promise<SpoofingAnalysis> {
   ...
   */
  private static async analyzeReflections(photo: CapturedPhoto): Promise<number> {
    try {
  ...
  private static analyzeDepthConsistency(landmarks?: LandmarkPoint[]): number {
    try {
      ...
      const landmarksWithDepth = landmarks.filter(point => point.z !== undefined);
   ...
   */
  private static analyzeLightingConsistency(faceData: FaceDetectionData): number {
    try {
      // Analyze lighting consistency using eye openness as a proxy
      const eyeOpenness = (faceData.leftEyeOpenProbability + faceData.rightEyeOpenProbability) / 2;
      ...
      const angleConsistency = 1 - (Math.abs(faceData.rollAngle) + Math.abs(faceData.yawAngle)) / 180;
  ...
  static quickSpoofingCheck(faceData: FaceDetectionData): boolean {
    try {
      ...
      if (centeredness < 0.05) { // Too centered
        return true; // Potentially spoofed
      }
      
      // 2. Perfect angles (real faces rarely have perfect 0-degree angles)
      if (Math.abs(faceData.rollAngle) < 0.5 && Math.abs(faceData.yawAngle) < 0.5) {
        return true; // Potentially spoofed
      }
