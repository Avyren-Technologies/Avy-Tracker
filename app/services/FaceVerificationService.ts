...
const VERIFICATION_CONFIDENCE_THRESHOLD = 0.7; // Adjusted threshold for compatibility mode (70% confidence)
...
export const generateEnhancedVerificationResult = async (
  ...
  try {
    ...
    const spoofingAnalysis = await AntiSpoofingService.analyzeImage(
      photo,
      faceData,
    );
    ...
    return {
      success:
        confidence >= VERIFICATION_CONFIDENCE_THRESHOLD &&
        !spoofingAnalysis.isSpoofed,
      confidence,
      livenessDetected,
      timestamp: new Date(),
      factors,
...
const calculateQualityScore = (
  ...
  const eyeScore =
    (faceData.leftEyeOpenProbability + faceData.rightEyeOpenProbability) / 2;
  const angleScore =
 ...
 */
const calculateFacialMeasurements = (
  faceData: FaceDetectionData,
  photo: CapturedPhoto,
): number[] => {
  ...
  if (!faceData.landmarks || faceData.landmarks.length === 0) {
    ...
    console.error(
      "❌ CRITICAL: No landmarks available from MLKit - cannot generate valid face encoding",
    );
    throw ErrorHandlingService.createError(
      FaceVerificationErrorType.PROCESSING_ERROR,
      new Error(
        "MLKit failed to extract facial landmarks - face encoding cannot be generated",
      ),
    );
  }
  ...
  try {
    ...
    console.log(
      `🔍 Landmark count: ${landmarkCount}, adapting measurement strategy`,
    );
    ...
    const getLandmarkPosition = (index: number): any => {
      if (
        faceData.landmarks &&
        index >= 0 &&
        index < faceData.landmarks.length
      ) {
      ...
      console.warn(
        `⚠️ Landmark index ${index} out of bounds (max: ${faceData.landmarks?.length ? faceData.landmarks.length - 1 : 0})`,
      );
      return null;
    };
    ...
    const leftEyebrowLeft = getLandmarkPosition(
      LANDMARK_INDICES.LEFT_EYEBROW_LEFT,
    );
    const leftEyebrowRight = getLandmarkPosition(
      LANDMARK_INDICES.LEFT_EYEBROW_RIGHT,
    );
    const rightEyebrowLeft = getLandmarkPosition(
      LANDMARK_INDICES.RIGHT_EYEBROW_LEFT,
    );
    const rightEyebrowRight = getLandmarkPosition(
      LANDMARK_INDICES.RIGHT_EYEBROW_RIGHT,
    );
    ...
    const leftEyeWidth = calculateNormalizedDistance(
      leftEyebrowLeft,
      leftEyebrowRight,
    );
    const rightEyeWidth = calculateNormalizedDistance(
      rightEyebrowLeft,
      rightEyebrowRight,
    );
    ...
    const leftEyeSymmetry =
      Math.abs(leftEyeWidth - rightEyeWidth) /
    ...
    measurements.push(
      ...
      ...Array(35)
        ...
        .map((_, i) => {
          ...
          return (
            Math.min(faceData.bounds.width, faceData.bounds.height) /
            Math.max(faceData.bounds.width, faceData.bounds.height)
          );
        }),
    );
    ...
    console.log("📏 Facial measurements calculated:", {
      ...
      nonZeroMeasurements: measurements.filter((m) => m !== 0 && m !== 0.5)
        .length,
    ...
    if (measurements.length < 50) {
      console.error(
        "❌ CRITICAL: Insufficient measurements calculated - cannot generate valid face encoding",
      );
      throw ErrorHandlingService.createError(
        FaceVerificationErrorType.PROCESSING_ERROR,
        new Error(
          `Only ${measurements.length} measurements calculated, expected 50`,
        ),
      );
    }
    ...
    throw ErrorHandlingService.createError(
      FaceVerificationErrorType.PROCESSING_ERROR,
      new Error(
        `Failed to calculate facial measurements: ${error instanceof Error ? error.message : String(error)}`,
      ),
    );
  }
...
export const generateFaceEncoding = async (
  ...
  try {
    ...
    if (
      !faceData.bounds ||
      faceData.bounds.width <= 0 ||
      faceData.bounds.height <= 0
    ) {
      throw ErrorHandlingService.createError(
    ...
    if (faceData.landmarks && faceData.landmarks.length > 0) {
      try {
        const landmarkFeatures = faceData.landmarks
          .map((point) => {
            if (
              !point ||
              ...
              const faceCenterY =
                faceData.bounds.y + faceData.bounds.height / 2;
            ...
            const normalizedY = Math.max(
              0,
              Math.min(1, point.y / photo.height),
            );
        ...
        console.log(`🔍 MLKit landmark analysis:`, {
          ...
          landmarkModel:
            actualLandmarkCount === 20
              ? "10-point"
              : actualLandmarkCount === 136
                ? "68-point"
                : actualLandmarkCount === 936
                  ? "468-point"
                  : "unknown",
        });
        ...
        if (actualLandmarkCount < 20) {
          console.error(
            "❌ CRITICAL: Insufficient landmarks from MLKit - cannot generate valid face encoding",
          );
          throw ErrorHandlingService.createError(
            FaceVerificationErrorType.PROCESSING_ERROR,
            new Error(
              `MLKit provided only ${actualLandmarkCount} landmark features, minimum 20 required`,
            ),
          );
        }
        ...
        if (actualLandmarkCount < expectedLandmarkCount) {
          console.warn(
            `⚠️ MLKit provided ${actualLandmarkCount} landmarks (${actualLandmarkCount / 2}-point model), adapting encoding strategy`,
          );
          ...
          const adaptedLandmarkCount = Math.floor(
            actualLandmarkCount * (1 + landmarkRatio),
          ); // Slightly expand but not to full size
          ...
          console.log(
            `✅ Adapted landmark encoding: ${actualLandmarkCount} → ${landmarkFeatures.length} features`,
          );
        } else {
        ...
        console.log("✅ Landmark features extracted:", {
          count: landmarkFeatures.length,
        });
      } catch (error) {
        console.error(
          "❌ CRITICAL: Error extracting landmark features:",
          error,
        );
        throw ErrorHandlingService.createError(
          FaceVerificationErrorType.PROCESSING_ERROR,
          new Error(
            `Failed to extract landmark features: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }
    } else {
    ...
    console.log("📐 Geometric features calculated:", {
      ...
      facePosition: {
        x: (faceData.bounds.x / photo.width).toFixed(4),
        y: (faceData.bounds.y / photo.height).toFixed(4),
      },
      faceSize: {
        width: (faceData.bounds.width / photo.width).toFixed(4),
        height: (faceData.bounds.height / photo.height).toFixed(4),
      },
      eyeProbabilities: {
        left: faceData.leftEyeOpenProbability.toFixed(4),
        right: faceData.rightEyeOpenProbability.toFixed(4),
      },
      angles: {
    ...
    if (faceData.attributes) {
      const attributeFeatures = [
        ...
        faceData.attributes.gender === "male"
          ? 1
          : faceData.attributes.gender === "female"
            ? 0
            : 0.5,
        faceData.attributes.headEulerAngles?.x
          ? Math.sin((faceData.attributes.headEulerAngles.x * Math.PI) / 180)
          : 0,
        faceData.attributes.headEulerAngles?.y
          ? Math.sin((faceData.attributes.headEulerAngles.y * Math.PI) / 180)
          : 0,
        faceData.attributes.headEulerAngles?.z
          ? Math.sin((faceData.attributes.headEulerAngles.z * Math.PI) / 180)
          : 0,
      ];
    ...
    if (validFeatures.length !== features.length) {
      console.warn(
        "⚠️ Invalid features detected, replacing with neutral values",
      );
    ...
    const expectedDimensions =
      actualDimensions >= FACE_ENCODING_DIMENSIONS
        ? FACE_ENCODING_DIMENSIONS
        : actualDimensions;
    ...
    try {
      ...
      throw ErrorHandlingService.createError(
        FaceVerificationErrorType.ENCODING_GENERATION_FAILED,
        new Error(
          `Base64 encoding failed: ${encodingError instanceof Error ? encodingError.message : String(encodingError)}`,
        ),
      );
    }
  } catch (error) {
 ...
 */
const calculateEuclideanSimilarity = (
 ...
 * Returns similarity score between 0 and 1 (higher = more similar)
 */
const calculateManhattanSimilarity = (
  features1: Float32Array,
  features2: Float32Array,
): number => {
  // CRITICAL FIX: Handle edge cases gracefully
 ...
 */
const compareFaceEncodingsSimple = (
  encoding1: string,
  encoding2: string,
): number => {
  try {
 ...
 */
const calculateEnhancedSimilarity = (
  features1: Float32Array,
  features2: Float32Array,
): number => {
  ...
  const enhancedSimilarity =
    cosineSim * 0.5 + euclideanSim * 0.3 + manhattanSim * 0.2;
...
export const compareFaceEncodings = (
  ...
  try {
    ...
    if (maxLength > minLength * 2) {
      console.warn(
        "Major encoding dimension mismatch detected - using compatibility mode",
      );
    ...
    if (
      encodingLength <
      landmarkCount + geometricCount + measurementCount + attributeCount
    ) {
    ...
    try {
      ...
      const hasValidLandmarks1 = landmarkFeatures1
        .slice(0, realLandmarkCount)
        .some((val) => val !== 0 && val > 0.001);
      const hasValidLandmarks2 = landmarkFeatures2
        .slice(0, realLandmarkCount)
        .some((val) => val !== 0 && val > 0.001);

      if (hasValidLandmarks1 && hasValidLandmarks2) {
        landmarkSimilarity = calculateCosineSimilarity(
          landmarkFeatures1,
          landmarkFeatures2,
        );
      } else {
        console.warn(
          "⚠️ CRITICAL: Landmarks are invalid or zero values, rejecting verification",
        );
      ...
      const geometricFeatures1 = features1.slice(
        landmarkCount,
        landmarkCount + geometricCount,
      );
      const geometricFeatures2 = features2.slice(
        landmarkCount,
        landmarkCount + geometricCount,
      );

      // Check if geometric features are valid (not all neutral values)
      const hasValidGeometric1 = geometricFeatures1.some(
        (val) => val !== 0.5 && val !== 0,
      );
      const hasValidGeometric2 = geometricFeatures2.some(
        (val) => val !== 0.5 && val !== 0,
      );

      if (hasValidGeometric1 && hasValidGeometric2) {
        geometricSimilarity = calculateCosineSimilarity(
          geometricFeatures1,
          geometricFeatures2,
        );
      } else {
        console.warn(
          "⚠️ CRITICAL: Geometric features are placeholder values, rejecting verification",
        );
      ...
      const measurementFeatures1 = features1.slice(
        landmarkCount + geometricCount,
        landmarkCount + geometricCount + measurementCount,
      );
      const measurementFeatures2 = features2.slice(
        landmarkCount + geometricCount,
        landmarkCount + geometricCount + measurementCount,
      );

      // Check if measurement features are valid (not all neutral values)
      const hasValidMeasurements1 = measurementFeatures1.some(
        (val) => val !== 0.5 && val !== 0,
      );
      const hasValidMeasurements2 = measurementFeatures2.some(
        (val) => val !== 0.5 && val !== 0,
      );

      if (hasValidMeasurements1 && hasValidMeasurements2) {
        measurementSimilarity = calculateCosineSimilarity(
          measurementFeatures1,
          measurementFeatures2,
        );
      } else {
        console.warn(
          "⚠️ CRITICAL: Measurement features are placeholder values, rejecting verification",
        );
      ...
      const attributeFeatures1 = features1.slice(
        landmarkCount + geometricCount + measurementCount,
        landmarkCount + geometricCount + measurementCount + attributeCount,
      );
      const attributeFeatures2 = features2.slice(
        landmarkCount + geometricCount + measurementCount,
        landmarkCount + geometricCount + measurementCount + attributeCount,
      );

      // Check if attribute features are valid (not all neutral values)
      const hasValidAttributes1 = attributeFeatures1.some(
        (val) => val !== 0.5 && val !== 0,
      );
      const hasValidAttributes2 = attributeFeatures2.some(
        (val) => val !== 0.5 && val !== 0,
      );

      if (hasValidAttributes1 && hasValidAttributes2) {
        attributeSimilarity = calculateCosineSimilarity(
          attributeFeatures1,
          attributeFeatures2,
        );
      } else {
        console.warn(
          "⚠️ CRITICAL: Attribute features are placeholder values, rejecting verification",
        );
      ...
      const safeLandmarkSimilarity = isNaN(landmarkSimilarity)
        ? 0.5
        : Math.max(0, landmarkSimilarity);
      const safeGeometricSimilarity = isNaN(geometricSimilarity)
        ? 0.5
        : Math.max(0, geometricSimilarity);
      const safeMeasurementSimilarity = isNaN(measurementSimilarity)
        ? 0.5
        : Math.max(0, measurementSimilarity);
      const safeAttributeSimilarity = isNaN(attributeSimilarity)
        ? 0.5
        : Math.max(0, attributeSimilarity);
      ...
      if (!geometricValid) {
        ...
        geometricWeight = 0;
        console.log(
          "🔄 Geometric features invalid, redistributing weight to landmarks",
        );
      }
      ...
      if (!measurementValid) {
        ...
        measurementWeight = 0;
        console.log(
          "🔄 Measurement features invalid, redistributing weight to landmarks",
        );
      }
      ...
      const hasRealFeatures =
        ...
        hasValidGeometric2;
      if (
        safeLandmarkSimilarity > 0.99 &&
        hasRealFeatures &&
        (!geometricValid || !measurementValid)
      ) {
        ...
        console.log(
          `🚀 Confidence boosted due to excellent landmarks: ${(overallSimilarity - boost).toFixed(4)} → ${overallSimilarity.toFixed(4)}`,
        );
      } else if (safeLandmarkSimilarity > 0.99 && !hasRealFeatures) {
        console.warn(
          "⚠️ CRITICAL: High landmark similarity detected but features are placeholders - rejecting verification",
        );
      ...
      console.log("Enhanced face comparison results:", {
        ...
        encodingLengths: {
          encoding1: features1.length,
          encoding2: features2.length,
        },
        featureCounts: {
          landmarks: landmarkCount,
          geometric: geometricCount,
      ...
      console.warn(
        "Multi-factor comparison failed, falling back to enhanced similarity:",
        error,
      );
...
const generateEncryptionKey = async (): Promise<string> => {
  try {
    ...
    return generateUUID()
      .replace(/-/g, "")
      .substring(0, ENCRYPTION_CONFIG.keyLength * 2);
  }
};
...
const generateSalt = async (): Promise<string> => {
  try {
    ...
    return generateUUID()
      .replace(/-/g, "")
      .substring(0, ENCRYPTION_CONFIG.saltLength * 2);
  }
};
...
export const encryptFaceData = async (
  ...
  try {
    ...
    let deviceKey = await SecureStore.getItemAsync(
      "device_face_encryption_key",
    );
    if (!deviceKey) {
...
export const decryptFaceData = async (
  ...
  console.warn(
    "decryptFaceData is deprecated - use verifyFaceData for data verification",
  );
  throw ErrorHandlingService.createError(
    FaceVerificationErrorType.PROCESSING_ERROR,
    ...
    async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            "Failed to cleanup profile after encoding storage failure:",
            cleanupError,
          );
        }
      ...
      try {
        await OfflineVerificationService.cacheFaceProfile(
          userId,
          encodingHash,
          encryptedData,
        );
        ...
        console.warn(
          "Failed to cache face profile for offline use:",
          cacheError,
        );
 ...
 */
export const getFaceProfile = async (
  userId: number,
): Promise<StoredFaceProfile | null> => {
  try {
 ...
 */
export const getFaceEncoding = async (
  userId: number,
): Promise<string | null> => {
  try {
...
const performOnlineVerification = async (
  ...
  try {
    ...
    if (!storedEncoding) {
      ...
      error.userMessage =
        "No face profile found. Please register your face first before starting a shift.";
      throw error;
    }
    ...
    const confidence = compareFaceEncodings(
      storedEncoding,
      currentEncoding,
      context,
    );
    ...
    if (isNaN(confidence) || !isFinite(confidence)) {
      ...
      console.log(
        "🔍 Attempting fallback comparison with simple cosine similarity...",
      );

      // Fallback: try simple cosine similarity
      try {
        const fallbackConfidence = compareFaceEncodingsSimple(
          storedEncoding,
          currentEncoding,
        );
        ...
        throw ErrorHandlingService.createError(
          FaceVerificationErrorType.PROCESSING_ERROR,
          new Error(
            "Face comparison calculation failed - invalid confidence value",
          ),
        );
      }
    }
    ...
    if (currentPhoto && currentFaceData) {
      ...
      spoofingAnalysis = await AntiSpoofingService.analyzeImage(
        currentPhoto,
        currentFaceData,
      );
    } else {
      console.log(
        "⚠️ Skipping anti-spoofing analysis - missing photo or face data",
      );
    ...
    if (spoofingAnalysis.isSpoofed) {
      ...
      throw ErrorHandlingService.createError(
        ...
        new Error(
          `Anti-spoofing analysis failed: overall score ${spoofingAnalysis.overallScore.toFixed(2)} below threshold 0.7`,
        ),
        context,
      );
    }
    ...
    if (confidence <= VERIFICATION_CONFIDENCE_THRESHOLD - tolerance) {
      ...
      const error = ErrorHandlingService.createError(
        errorType,
        new Error(
          `Verification confidence (${confidence.toFixed(2)}) below threshold (${VERIFICATION_CONFIDENCE_THRESHOLD})`,
        ),
        context,
      );
    ...
    const isCompatibilityMode =
      storedEncoding.length !== currentEncoding.length;
    const effectiveThreshold = isCompatibilityMode
      ? 0.65
    ...
    const success =
      confidence >= effectiveThreshold && !spoofingAnalysis.isSpoofed;
...
const performOfflineVerification = async (
  ...
  try {
    ...
    const offlineResult = await OfflineVerificationService.verifyFaceOffline(
      userId,
      currentEncoding,
    );
    ...
    if (offlineResult.requiresOnlineVerification) {
      throw ErrorHandlingService.createError(
        ...
        new Error(
          "Offline verification not available - cached data expired or not found",
        ),
      );
    }
    ...
    if (offlineResult.confidence < VERIFICATION_CONFIDENCE_THRESHOLD) {
      throw ErrorHandlingService.createError(
        ...
        new Error(
          `Offline verification confidence (${offlineResult.confidence.toFixed(2)}) below threshold`,
        ),
      );
    }
    ...
    if (error && typeof error === "object" && "type" in error) {
      ...
      if (error.type === FaceVerificationErrorType.NETWORK_ERROR) {
        console.warn(
          "OfflineVerificationService failed, attempting local fallback",
        );
        return await performLocalFallbackVerification(
          userId,
          currentEncoding,
          livenessDetected,
          context,
    ...
    console.warn(
      "OfflineVerificationService failed, falling back to local verification:",
      error,
    );
    return await performLocalFallbackVerification(
      userId,
  ...
  try {
    ...
    if (confidence < VERIFICATION_CONFIDENCE_THRESHOLD) {
      throw ErrorHandlingService.createError(
        FaceVerificationErrorType.LOW_CONFIDENCE,
        new Error(
          `Local verification confidence (${confidence.toFixed(2)}) below threshold`,
        ),
      );
    }
 ...
 */
export const getCachedVerifications = async (
  userId: number,
 ...
 */
export const getOfflineVerifications = async (): Promise<
  OfflineVerification[]
> => {
  try {
 ...
 */
export const markVerificationsAsSynced = async (
 ...
 */
export const syncOfflineVerifications = async (
  apiEndpoint: string,
  authToken: string,
): Promise<void> => {
  try {
    ...
    if (response.ok) {
      ...
      const syncedIds =
        result.syncedIds || offlineVerifications.map((v) => v.id);
 ...
 */
export const getFaceRegistrationStatus = async (
  userId: number,
  ...
  try {
    ...
    return {
      ...
      lastVerification: profile.lastUpdated
 ...
 */
export const cacheGeofencesForOfflineUse = async (
  geofences: {
    id: string;
    name: string;
 ...
 */
export const isCachedDataAvailable = async (
  userId: number,
): Promise<{
  available: boolean;
  expiresAt?: Date;
