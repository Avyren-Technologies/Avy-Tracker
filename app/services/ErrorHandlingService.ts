...
export class ErrorHandlingService {
  ...
  public getErrorStats(): {
    ...
    this.errorLogs.forEach((log) => {
      ...
      if (log.error && "type" in log.error) {
        ...
        if (faceError.type) {
          errorsByType[faceError.type] =
            (errorsByType[faceError.type] || 0) + 1;
        }
        if (faceError.severity) {
          errorsBySeverity[faceError.severity] =
            (errorsBySeverity[faceError.severity] || 0) + 1;
        }
      }
    });
  ...
  public static mapErrorToFaceVerificationError(
    error: Error,
  ): FaceVerificationError {
    return {
  ...
  private static getUserMessageForType(
    ...
    const messages: Record<FaceVerificationErrorType, string> = {
      ...
      [FaceVerificationErrorType.CAMERA_HARDWARE_ERROR]:
        "Camera hardware error. Please restart the app.",
      ...
      [FaceVerificationErrorType.FACE_TOO_SMALL]:
        "Face appears too small. Please move closer to the camera.",
      [FaceVerificationErrorType.FACE_TOO_LARGE]:
        "Face appears too large. Please move further from the camera.",
      [FaceVerificationErrorType.FACE_NOT_CENTERED]:
        "Face not centered. Please position your face in the center.",
      [FaceVerificationErrorType.FACE_ANGLE_INVALID]:
        "Face angle not suitable. Please look straight at the camera.",
      [FaceVerificationErrorType.POOR_LIGHTING]:
        "Poor lighting detected. Please move to a well-lit area.",
      [FaceVerificationErrorType.TOO_BRIGHT]:
        "Lighting too bright. Please move to a less bright area.",
      [FaceVerificationErrorType.TOO_DARK]:
        "Lighting too dark. Please move to a brighter area.",
      [FaceVerificationErrorType.BLURRY_IMAGE]:
        "Image is blurry. Please hold the device steady.",
      [FaceVerificationErrorType.LOW_IMAGE_QUALITY]:
        "Image quality is low. Please try again.",
      [FaceVerificationErrorType.NO_LIVENESS_DETECTED]:
        "No liveness detected. Please blink or move slightly.",
      [FaceVerificationErrorType.LIVENESS_TIMEOUT]:
        "Liveness detection timed out. Please try again.",
      [FaceVerificationErrorType.INSUFFICIENT_MOVEMENT]:
        "Insufficient movement detected. Please blink naturally.",
      [FaceVerificationErrorType.FAKE_FACE_DETECTED]:
        "Potential fake face detected. Please use a real face.",
      [FaceVerificationErrorType.LOW_CONFIDENCE]:
        "Low confidence in verification. Please try again.",
      ...
      [FaceVerificationErrorType.ENCODING_GENERATION_FAILED]:
        "Failed to generate face encoding. Please try again.",
      ...
      [FaceVerificationErrorType.SYNC_ERROR]:
        "Sync error. Data will be synced when connection is restored.",
      [FaceVerificationErrorType.TOO_MANY_ATTEMPTS]:
        "Too many attempts. Please wait before trying again.",
      [FaceVerificationErrorType.ACCOUNT_LOCKED]:
        "Account temporarily locked. Please contact support.",
      ...
      [FaceVerificationErrorType.MEMORY_ERROR]:
        "Memory error. Please restart the app.",
  ...
  private static getSuggestionsForType(
    ...
    return (
      suggestions[type] || ["Try again", "Check settings", "Contact support"]
    );
  }

  // Get severity for error type
  private static getSeverityForType(
    type: FaceVerificationErrorType,
  ...
  public static getRecoveryActions(
  ...
  // Should use fallback
  public static shouldUseFallback(
    error: Error | FaceVerificationError,
    attemptCount: number,
  ): boolean {
    const maxAttempts = 3;
    ...
    if ("type" in error) {
      ...
      // Regular Error
      const isCritical =
        (error as any).critical || error.name.includes("Critical");
  ...
  public static formatErrorForUser(error: Error | FaceVerificationError):
    | string
    | {
        title: string;
        message: string;
