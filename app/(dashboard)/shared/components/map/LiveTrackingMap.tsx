...
interface LiveTrackingMapProps {
  ...
  userRoutes?: Record<string, { latitude: number; longitude: number }[]>;
...
const LiveTrackingMap: React.FC<LiveTrackingMapProps> = ({
  ...
  const trackedUsers = useLocationTrackingStore(
    (state: any) => state.trackedUsers || [],
  );
  const storeGeofences = useLocationTrackingStore(
    (state: any) => state.geofences || [],
  );
  const userPaths = useLocationTrackingStore(
    (state: any) => state.userPaths || {},
  );
  const lastKnownUserLocations = useLocationTrackingStore(
    (state: any) => state.lastKnownUserLocations || {},
  );
  const selectedUser = useLocationTrackingStore(
    (state: any) => state.selectedUser,
  );
  const setSelectedUser = useLocationTrackingStore(
    (state: any) => state.setSelectedUser,
  );
  ...
  const currentGeofenceId = useLocationStore(
    (state) => state.currentGeofenceId,
  );
  ...
  const fitToUsers = useCallback(() => {
    ...
    console.log(
      `Found ${validLocations.length} valid locations out of ${locations.length} total locations`,
    );

    // If no valid locations after filtering, try device location
    if (validLocations.length === 0) {
      console.log(
        "No valid tracked locations - trying to find device location",
      );
      getCurrentLocation();
      return;
    }
    ...
    if (validLocations.length === 1) {
      ...
      mapRef.current?.animateToRegion(
        ...
        500,
      ); // Faster animation (500ms instead of default)
      return;
    }

    // Find min and max coordinates to create a bounding box
    const minLat = Math.min(
      ...validLocations.map((loc: LatLng) => loc.latitude),
    );
    const maxLat = Math.max(
      ...validLocations.map((loc: LatLng) => loc.latitude),
    );
    const minLng = Math.min(
      ...validLocations.map((loc: LatLng) => loc.longitude),
    );
  ...
  const handleLocatePress = useCallback(() => {
    ...
    if (user?.role === "employee") {
      ...
      if (!trackedUsers.length) {
        console.log(
          "No tracked users available, using current location instead",
        );
        getCurrentLocation();
      } else {
        fitToUsers();
  ...
  const region = useMemo<Region>(() => {
    ...
    if (
      ...
      console.log(
        "LiveTrackingMap - USING initialRegion from props:",
        initialRegion,
      );
    ...
    if (
      ...
      if (cacheAge < LOCATION_CACHE_MAX_AGE) {
        console.log(
          "LiveTrackingMap - USING cached location from Zustand:",
          cachedUserLocation,
        );
        return {
        ...
        console.log(
          "LiveTrackingMap - cached location too old, age:",
          cacheAge,
        );
      }
    }
    ...
    console.log(
      "LiveTrackingMap - USING Bangalore as fallback location:",
      bangaloreRegion,
    );
  ...
  useEffect(() => {
    ...
    const isNearZeroCoordinates =
      Math.abs(region.latitude) < 0.1 && Math.abs(region.longitude) < 0.1;
    ...
    if (isNearZeroCoordinates || isNearSanFrancisco) {
      ...
      requestCurrentLocation({
        ...
        if (location) {
          console.log(
            "LiveTrackingMap - found actual location, updating map:",
            location,
          );
          mapRef.current?.animateToRegion({
          ...
          console.log(
            "LiveTrackingMap - could not get location, using Bangalore fallback",
          );
          mapRef.current?.animateToRegion({
            latitude: 12.9716,
            longitude: 77.5946,
    ...
    try {
      if (
        ...
        if (__DEV__) {
          console.log("No geofences to render", {
            showGeofences,
            geofenceCount: displayGeofences?.length || 0,
          });
        }
        return null;
      }
      ...
      return displayGeofences.map((geofence: Geofence) => {
        try {
          ...
          if (!geofence || !geofence.coordinates) {
            console.warn(
              "Invalid geofence data (missing coordinates):",
              geofence?.id || "unknown",
            );
            return null;
          }
          ...
          if (geofence.coordinates.type === "Point") {
            try {
              ...
              if (
                ...
                console.warn(
                  "Invalid geofence coordinates array:",
                  geofence.id,
              ...
              if (isNaN(latitude) || isNaN(longitude)) {
                console.warn(
                  "Invalid geofence coordinates (NaN values):",
                  geofence.id,
                );
                return null;
              }
              ...
              console.error(
                "Error processing circle geofence:",
                innerError,
                geofence?.id,
              );
              return null;
          ...
          console.error(
            "Error processing individual geofence:",
            innerError,
            geofence?.id,
          );
          return null;
  ...
  const routesForDisplay = useMemo(() => {
    try {
      ...
      let routes: Record<string, LatLng[]> = {};
      ...
      Object.keys(routes).forEach((userId) => {
        ...
        if (route && route.length > 100) {
          ...
          if (simplifyFactor > 1) {
            const simplified = route.filter(
              (_, index) =>
                index % simplifyFactor === 0 || index === route.length - 1,
            );
  ...
  const getRouteStrokeWidth = useCallback(
    (userId: string) => {
  ...
  const getRouteZIndex = useCallback(
    (userId: string) => {
    ...
    >
      ...
      <View
        style={[
          ...
          colorScheme === "dark" ? styles.mapHeaderDark : styles.mapHeaderLight,
          { borderColor },
        ]}
        ...
        <View style={styles.mapHeaderContent}>
          <View style={styles.mapInfoSection}>
            ...
            <View style={styles.mapInfoDetail}>
              <Ionicons
                name={
                  user?.role === "employee" ? "map-outline" : "people-outline"
                }
                size={16}
                color={primaryColor}
      ...
      >
        ...
        {trackedUsers.map((user: TrackingUser) => {
          ...
          if (
            ...
            console.warn(
              "Skipping marker for user with invalid location:",
              user.id,
            );
            return null;
          }
          ...
          const markerColor =
            user.id === currentSelectedUserId
              ...
              : user.isActive
                ? "#3b82f6"
                : "#9ca3af"; // Blue for active, gray for inactive
        ...
        {showUserPaths &&
          Object.keys(routesForDisplay).map((userId) => {
            const route = routesForDisplay[userId];
            if (route && route.length > 1) {
              const isSelected =
                userId === currentSelectedUserId || userId === "currentUser";
              return (
                <Polyline
      ...
      {showControls && (
        ...
        >
          ...
          <View style={styles.controlWithLabel}>
            <TouchableOpacity
              style={[
                ...
                (user?.role === "employee"
                  ...
                  : !trackedUsers.length) && styles.disabledButton,
              ]}
            ...
            <Text
              style={[
                ...
                (user?.role === "employee"
                  ? employeeGeofences.length === 0
                  : !trackedUsers.length) && { opacity: 0.5 },
              ]}
            >
          ...
          <View style={styles.controlWithLabel}>
            ...
            <Text
              style={[styles.controlLabel, { color: textColor, marginTop: 4 }]}
            >
              My Location
            </Text>
          </View>
          ...
          <View style={styles.userDetailContent}>
            <View style={styles.userDetailRow}>
              <View style={styles.userDetailItem}>
                ...
                />
                <Text style={[styles.userDetailLabel, { color: textColor }]}>
                  Last Update:
                </Text>
                <Text style={[styles.userDetailValue, { color: textColor }]}>
                  {selectedUser.lastUpdated
                    ? formatTimeSince(selectedUser.lastUpdated)
                    : "Unknown"}
                </Text>
              </View>
