...
export default function EmployeeNotifications() {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      try {
        ...
        console.error(
          "[Employee Notifications] Error loading initial data:",
          error,
        );
      } finally {
  ...
  const filterTypes = useMemo(() => {
    ...
    const taskCount =
      notifications?.filter(
        (n: Notification) => n.type === "task-assignment" && !n.read,
      ).length || 0;
    const leaveCount =
      notifications?.filter(
        (n: Notification) => n.type === "leave-status" && !n.read,
      ).length || 0;
    const expenseCount =
      notifications?.filter(
        (n: Notification) => n.type === "expense-status" && !n.read,
      ).length || 0;
    const generalCount =
      notifications?.filter(
        (n: Notification) => n.type === "general" && !n.read,
      ).length || 0;
  ...
  const renderFilterTab = useCallback(
    (
      type: { id: string; label: string; icon: string; count: number },
      index: number,
    ) => (
      <Pressable
        key={type.id}
