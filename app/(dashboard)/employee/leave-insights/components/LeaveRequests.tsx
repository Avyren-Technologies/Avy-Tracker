...
export default function LeaveRequests() {
  ...
  const [loadingDocumentId, setLoadingDocumentId] = useState<number | null>(
    null,
  ); // Track loading state for specific document
  ...
  const fetchData = async (isRefreshing = false) => {
    try {
      ...
      const [requestsResponse, balanceResponse, typesResponse] =
        await Promise.all([
  ...
  const handleViewDocument = async (document: {
    ...
    try {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/api/leave/document/${document.id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      const file = new File(Paths.cache, fetchedDocument.file_name);
      ...
      await file.create();
      await FileSystem.writeAsStringAsync(file.uri, fetchedDocument.file_data, {
        encoding: 'base64',
      });
      ...
      if (Platform.OS === "android") {
        ...
        if (canShare) {
          await Sharing.shareAsync(file.uri, {
            mimeType: fetchedDocument.file_type,
            dialogTitle: `Open ${fetchedDocument.file_name}`,
          });
        } else {
        ...
        try {
          ...
          if (canShare) {
            await Sharing.shareAsync(file.uri, {
              UTI:
                fetchedDocument.file_type === 'application/pdf'
                  ? 'com.adobe.pdf'
                  : 'public.item',
              mimeType: fetchedDocument.file_type,
            });
    ...
    <View className="flex-1">
      ...
      <View className="flex-row items-center justify-between mb-4">
        ...
        >
          ...
          />
          <Text
            className={`font-semibold ml-1 text-base ${
