...
const ChatMessage = memo(
  ({ message, isUser, timestamp, isDark, isStreaming }: ChatMessageProps) => {
    ...
    return (
      <View className={`px-4 py-2 ${isUser ? "items-end" : "items-start"}`}>
        <View
          className={`flex-row items-end gap-3 ${isUser ? "flex-row-reverse" : ""}`}
        >
          {!isUser && (
            <View
              className={`rounded-full ${isDark ? "bg-gray-700" : "bg-gray-100"} p-2.5`}
            >
              <Ionicons
          ...
          <View
            className={`rounded-2xl px-4 py-3 ${
              isUser ? "bg-blue-500" : isDark ? "bg-gray-800" : "bg-white"
            } shadow-sm`}
            style={{
              maxWidth: Math.min(
                width * (isUser ? 0.75 : 0.85),
                isUser ? 400 : 600,
              ),
              minWidth: 50,
              width: "auto",
