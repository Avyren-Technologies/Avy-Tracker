...
export default function LiveChat() {
  ...
  const cleanupOldMessages = useCallback(async () => {
    try {
      ...
      setMessages((prevMessages) =>
        prevMessages.filter(
          (msg) => new Date(msg.created_at) > thirtyMinutesAgo,
        ),
      );
  ...
  const loadChatHistory = async () => {
    try {
      ...
      const historyWithIds = response.data
        ...
        .concat(
          response.data.map((msg: any, index: number) => ({
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.filter((msg) => msg.id !== `typing-${newMessageId}`),
      );
    } finally {
      setIsLoading(false);
    }
