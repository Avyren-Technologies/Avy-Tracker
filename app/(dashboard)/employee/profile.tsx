...
export default function Profile() {
  ...
  const fetchProfileStats = async (forceRefresh = false) => {
    try {
      if (!forceRefresh) {
        ...
        const cachedActivities = await AsyncStorage.getItem(
          CACHE_KEYS.PROFILE_ACTIVITIES,
        );
      ...
      const formattedActivities = response.data.recentActivities.map(
        (activity: any) => ({
          type: activity.type,
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      await Promise.all([fetchProfileImage(true), fetchProfileStats(true)]);
    } catch (error) {
  ...
  return (
    <View className="flex-1">
      ...
      >
        {/* Profile Info */}
        <View
          className={`m-4 p-4 rounded-lg ${isDark ? "bg-gray-800" : "bg-white"}`}
          style={styles.card}
        >
          <View className="items-center mb-4">
            {profileImage ? (
              ...
              <View
                className="w-24 h-24 rounded-full bg-blue-500 items-center justify-center mb-3"
                style={styles.profileImage}
              >
            ...
            <View className="flex-col items-center mt-2">
              <View
                className={`px-3 py-1 rounded-full mb-2 ${isDark ? "bg-blue-900" : "bg-blue-100"}`}
              >
            ...
            <View key={index} className="w-1/2 p-2">
              ...
              >
                <View className="flex-row items-center justify-between mb-2">
                  <View
                    className={`p-2 rounded-full opacity-20`}
