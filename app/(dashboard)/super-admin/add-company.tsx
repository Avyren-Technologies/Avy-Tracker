...
export default function AddCompany() {
  ...
  const validateForm = () => {
    ...
    if (!formData.managementName)
      newErrors.managementName = "Management name is required";
    if (!formData.managementEmployeeNumber)
      newErrors.managementEmployeeNumber = "Employee number is required";
    if (
      !formData.managementEmail ||
      !/\S+@\S+\.\S+/.test(formData.managementEmail)
    ) {
      newErrors.managementEmail = "Valid management email is required";
    }
    if (
      !formData.managementPhone ||
      !/^\+91\d{10}$/.test(formData.managementPhone)
    ) {
      newErrors.managementPhone = "Valid 10-digit phone number is required";
    }
    if (
      !formData.managementPassword ||
      formData.managementPassword.length < 8
    ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response?.data?.details?.includes("users_phone_key")) {
        ...
        error.response?.data?.details?.includes("users_employee_number_key")
      ) {
        errorMessage =
          "This employee number is already registered. Please use a different employee number.";
        setErrors((prev) => ({
          ...prev,
                ...
                {section.fields.map((field) => (
                  <View key={field.key} className="mb-4">
                    ...
                    {field.isImage ? (
                      ...
                      <View
                        className={`rounded-lg ${errors[field.key] ? "border-2 border-red-500" : "border border-gray-200"}`}
                        style={[
                      ...
                      >
                        <View className="flex-row items-center flex-1">
                          <View className="absolute left-4 z-10 h-full justify-center">
                            <Ionicons
                              name={
                                field.icon as keyof typeof Ionicons.glyphMap
                              }
                              size={20}
                              color={theme === "dark" ? "#9CA3AF" : "#6B7280"}
                            />
                      ...
                      <View className="relative">
                        ...
                        <View className="flex-row items-center flex-1">
                          ...
                          <TextInput
                            value={formData[
                              field.key as keyof CompanyFormData
                            ]?.replace(/^\+91/, "")}
                            onChangeText={(text) =>
                              handlePhoneChange(text, field.key)
                            }
