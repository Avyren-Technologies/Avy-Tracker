...
export default function TrackingTestScreen() {
  ...
  const checkPermissions = async () => {
    try {
      ...
      setPermissionStatus(
        `Foreground: ${foregroundPermission.status}, Background: ${backgroundPermission.status}`,
      );
      addLog(
        `Permissions - Foreground: ${foregroundPermission.status}, Background: ${backgroundPermission.status}`,
      );
    } catch (error) {
  ...
  const requestBackgroundPermission = async () => {
    ...
    try {
      ...
      addLog(
        `❌ Error requesting background permission: ${error.message || String(error)}`,
      );
