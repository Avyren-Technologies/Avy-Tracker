...
export default function LeaveBalanceManager() {
  ...
  const fetchUserBalances = async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        const processedBalances = response.data.map((balance) => ({
        ...
        console.log(
          "Processed balances:",
          JSON.stringify(processedBalances, null, 2),
        );
        ...
        Alert.alert(
          "Error",
          "Invalid balance data format received from server",
        );
      }
    } catch (error) {
      ...
      Alert.alert("Error", "Failed to fetch leave balances. Please try again.");
    } finally {
      setLoading(false);
    }
  };
            ...
            try {
              ...
              if (response.data.success) {
                ...
                Alert.alert(
                  "Error",
                  response.data.error ||
                    "Failed to initialize default balances",
                );
              }
            } catch (error) {
  ...
  const handleUpdateBalance = async () => {
    ...
    try {
      ...
      if (isNaN(carryForwardDays) || carryForwardDays < 0) {
        Alert.alert(
          "Error",
          "Please enter a valid number for carry forward days",
        );
        return;
      }
  ...
  const filteredUsers = users.filter((user) => {
    const matchesSearch =
      ...
      (user.employee_number?.toLowerCase() || "").includes(
    ...
    <View className="flex-1">
      ...
      <View className="flex-row justify-between items-center mb-4 px-4">
        ...
        <View className="flex-row items-center space-x-2">
          ...
          <View
            className={`px-3 py-1 rounded-lg ${isDark ? "bg-gray-800" : "bg-white"}`}
          >
        ...
        <View className="flex-row items-center space-x-2">
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 4 }}
            />
            <Text className="text-white">Initialize Defaults</Text>
          </TouchableOpacity>
            ...
            {selectedUser ? (
              <>
                <View className="flex-row items-center">
                  ...
                  <View
                    className={`ml-2 px-2 py-1 rounded-full ${getRoleBadgeColor(selectedUser.role).bg}`}
                  >
                    <Text
                      className={`text-xs ${getRoleBadgeColor(selectedUser.role).text}`}
                    >
                  ...
                  {selectedUser.gender && (
                    <View
                      className={`ml-2 px-2 py-1 rounded-full ${isDark ? "bg-gray-700" : "bg-gray-100"}`}
                    >
                      <Text
              ...
              <View className="flex-row justify-between items-center mb-2">
                ...
                <TouchableOpacity
                  onPress={() => {
                    ...
                    setNewCarryForwardDays(
                      balance.carry_forward_days.toString(),
                    );
      ...
      >
        <View
          className={`p-6 rounded-t-3xl ${isDark ? "bg-gray-900" : "bg-gray-50"}`}
        >
          <Text
