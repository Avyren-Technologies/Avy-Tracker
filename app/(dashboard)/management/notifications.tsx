...
export default function ManagementNotifications() {
  ...
  const sendNotification = async () => {
    try {
      ...
      const response = await axios.post(endpoint, notificationData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const loadNotifications = async () => {
      ...
      try {
        ...
        console.error(
          "[Management Notifications] Error loading initial data:",
          error,
        );
        if (isMounted.current) {
          setIsLoading(false);
  ...
  return (
    <View className="flex-1">
      ...
      <SendNotificationModal
        ...
        onRoleChange={(role) =>
          setNotificationData((prev) => ({ ...prev, targetRole: role }))
        }
        priority={notificationData.priority}
        onPriorityChange={(priority) =>
          setNotificationData((prev) => ({
            ...prev,
            priority: priority as "high" | "default" | "low",
          }))
        }
        selectedUsers={notificationData.userIds}
        onUserSelect={(userIds) =>
          setNotificationData((prev) => ({ ...prev, userIds }))
        }
