                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      Keyboard.dismiss();
                      onSelectEmployee(
                        item.id === 0 ? "all" : item.id.toString(),
                      );
                      onClose();
                    }}
                  >
    ...
    try {
      ...
      if (cachedData && lastFetch) {
        ...
        if (!isExpired) {
          ...
          const allAttendance = Object.values(attendance).reduce<
            AttendanceData[]
          >((acc, curr) => {
      ...
      setAttendanceData(attendance);
      const allAttendance = Object.values(attendance).reduce<AttendanceData[]>(
        (acc, curr) => {
          return acc.concat(curr as AttendanceData[]);
        },
        [],
          ...
          {[
            {
              ...
              value: isEmployeesLoading
                ? "-"
                : monthStats.totalEmployees.toString(),
              icon: "people-outline",
              color: "bg-blue-500",
            ...
            <View key={index} className="w-1/2 p-2">
              ...
              >
                ...
                {stat.isLoading ? (
                  ...
                  <>
                    <Text
                      className={`mt-2 text-2xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}
                    >
                      {stat.value}
                    </Text>
                    <Text
