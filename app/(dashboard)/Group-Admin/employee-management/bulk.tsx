      ...
      try {
        ...
        if (response.data.success && response.data.success.length > 0) {
          ...
          const successMessage =
            `Successfully processed ${summary?.total} employees:\n` +
            `✓ ${summary?.success} created successfully\n` +
            `✗ ${summary?.failed} failed`;

          if (response.data.errors && response.data.errors.length > 0) {
            // Group errors by type for better readability
            const errorsByType = response.data.errors.reduce(
              (acc: ErrorsByType, err: ErrorType) => {
            ...
            const errorDetails = (
              Object.entries(errorsByType) as [string, string[]][]
            )
              .map(([type, rows]) => `${type}:\n${rows.join("\n")}`)
              .join("\n\n");
              ...
              [
                {
                  ...
                  onPress: () => {
                    Alert.alert(
                      "Coming Soon",
                      "Error report download will be available in a future update.",
                    );
                  },
                },
                {
        ...
        if (error.response?.status === 400) {
          if (error.response.data.error === "User limit exceeded") {
            ...
            error.response.data.error === "Validation errors in CSV file"
          ) {
            // Group validation errors by type
            const errorsByType = error.response.data.errors.reduce(
              (acc: ErrorsByType, err: ErrorType) => {
                const errorType = err.error;
                if (!acc[errorType]) {
                  acc[errorType] = [];
        ...
        >
          ...
          >
            ...
            >
              ...
              {"\n"}
              John
              Doe,EMP001,john@example.com,+1234567890,Pass123!,IT,Developer,male,true
            </Text>
          </View>
