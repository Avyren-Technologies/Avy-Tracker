...
const MetricCard = ({
  ...
  return (
    <View
      className={`w-[48%] p-3 mb-3 rounded-lg ${isDark ? "bg-gray-800" : "bg-white"}`}
    >
      <View className="flex-row items-center mb-2">
        <View
          style={{ backgroundColor: `${color}20`, padding: 4, borderRadius: 8 }}
        >
          <Ionicons name={icon} size={16} color={color} />
        </View>
        <Text
          className={`ml-2 text-xs ${isDark ? "text-gray-300" : "text-gray-600"}`}
        >
          {title}
        </Text>
      </View>
      ...
      if (response.data) {
        ...
        const processMetrics = (data: any) => {
          return {
            ...
            on_time_rate: Math.min(
              100,
              Math.max(0, Number(data?.on_time_rate || 0)),
            ),
    ...
    switch (graphType) {
      case "bar":
        ...
        const maxAttendance = Math.max(
          ...analytics.daily.map((d) => Number(d.attendance_count || 0)),
        );
      ...
      case "line":
        const lineData = {
          ...
          datasets: [
            {
              data: analytics.weekly.map((w) =>
                Number(w.attendance_count || 0),
              ),
            },
          ],
        };
          ...
          <ScrollView className="p-4">
            ...
            <View className="mb-6">
              ...
              >
                <Text className={isDark ? "text-white" : "text-gray-800"}>
                  {filters.employeeId
                    ? analytics?.employees?.find(
                        (e) => e.id === filters.employeeId,
        ...
        >
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() =>
                  handleEmployeeSelect(item.id === 0 ? undefined : item.id)
                }
                className={`p-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"} ${
                  (item.id === 0 && !filters.employeeId) ||
                  item.id === filters.employeeId
                    ? isDark
                      ? "bg-indigo-900/30"
                      : "bg-indigo-100"
                    : ""
              ...
              <TouchableOpacity
                onPress={() =>
                  handleDepartmentSelect(
                    item.id === "all" ? undefined : item.id,
                  )
                }
                className={`p-4 border-b ${isDark ? "border-gray-700" : "border-gray-200"} ${
                  (item.id === "all" && !filters.department) ||
                  item.id === filters.department
                    ? isDark
                      ? "bg-indigo-900/30"
                      : "bg-indigo-100"
                    : ""
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View>
              ...
              >
                ...
                <Text
                  className={`w-16 px-3 font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}
                >
                  Days
                </Text>
                <Text
                  className={`w-20 px-3 font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}
                >
                  Status
                </Text>
              </View>
              ...
              {analytics.leave.map((leave, index) => (
                ...
                >
                  ...
                  <Text
                    className={`w-16 px-3 ${isDark ? "text-white" : "text-gray-800"}`}
                  >
                    {leave.daysCount}
                  </Text>
                  <Text
                    className={`w-20 px-3 ${isDark ? "text-white" : "text-gray-800"}`}
                  >
              ...
              <View className="mb-6">
                ...
                <View className="flex-row flex-wrap justify-between">
                  <MetricCard
                    ...
                    value={
                      analytics?.metrics?.total_employees?.toString() || "0"
                    }
                    icon="people"
                    color="#3B82F6"
                    isDark={isDark}
                  ...
                  <MetricCard
                    ...
                    value={
                      analytics?.metrics?.completed_shifts?.toString() || "0"
                    }
