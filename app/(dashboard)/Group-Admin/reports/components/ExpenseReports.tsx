...
export default function ExpenseReports({
  ...
  const [overallData, setOverallData] = useState<OverallExpenseData | null>(
    null,
  );
  ...
  const fetchEmployeeStats = async (employeeId?: string) => {
    try {
      ...
      if (response.data) {
        ...
        if (
          response.data.expenseStats &&
          Array.isArray(response.data.expenseStats)
        ) {
          const stats = response.data.expenseStats as ExpenseStat[];

          if (employeeId && employeeId !== "all") {
            const monthlyTotals = stats.reduce(
              (acc: { [key: string]: number }, curr: ExpenseStat) => {
                const month = curr.month || "Unknown";
                acc[month] =
                  (acc[month] || 0) + parseFloat(curr.total_amount || "0");
                return acc;
              },
              {},
            );
            ...
            const categoryTotals = stats.reduce(
              (acc: { [key: string]: number }, curr: ExpenseStat) => {
                const category = curr.category || "Other";
                acc[category] =
                  (acc[category] || 0) + parseFloat(curr.total_amount || "0");
                return acc;
              },
              {},
            );
            ...
            const average =
              monthlyData.length > 0 ? total / monthlyData.length : 0;
  ...
  const fetchExpenseData = async () => {
    try {
      ...
      if (response.data) {
        ...
        const processedCategoryData = response.data.categoryData.map(
  ...
  const resetFilters = () => {
    const defaultRange = dateRangeOptions.find(
      (option) => option.id === "last30Days",
    );
    if (defaultRange) {
  ...
  const renderFiltersModal = () => (
    ...
    >
      <View className="flex-1 bg-black/50">
        <View
          className={`flex-1 mt-24 rounded-t-3xl ${isDark ? "bg-gray-800" : "bg-white"}`}
        >
          ...
          <ScrollView className="p-4">
            <View className="mb-6">
              ...
              {filters.dateRangePreset === "custom" && (
                <View
                  className={`mt-2 p-4 rounded-lg ${isDark ? "bg-gray-700" : "bg-gray-100"}`}
                >
                  <View className="flex-row justify-between mb-2">
                    ...
                    >
                      ...
                      <View
                        className={`mt-1 p-2 rounded-lg ${isDark ? "bg-gray-600" : "bg-white"}`}
                      >
                        <Text
      ...
      {showDateRangePicker && (
        <DateTimePicker
          value={
            datePickerMode === "start" ? filters.startDate : filters.endDate
          }
          mode="date"
          display="default"
        ...
        <View className="h-[220px] justify-center items-center">
          ...
          >
            ...
            {selectedEmployee === "all"
              ? "any employee"
              : employees.find((emp) => emp.id.toString() === selectedEmployee)
    ...
    switch (graphType) {
      case "line":
        const chartData = {
          ...
          datasets: [
            {
              data: data.monthlyData.map(
                (item: MonthlyDataItem) => Number(item.amount) || 0,
              ),
            },
          ],
        };
      ...
      case "bar":
        ...
        return (
          <View>
            <BarChart
              data={{
                labels: data.categoryData.map(
                  (item: CategoryDataItem) => item.name,
                ),
                datasets: [
                  {
                    data: data.categoryData.map(
                      (item: CategoryDataItem) => Number(item.population) || 0,
                    ),
                  },
                ],
              }}
      ...
      case "pie":
        ...
        const total = data.categoryData.reduce(
          (sum: number, item: CategoryDataItem) =>
            sum + Number(item.population),
          0,
        );
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  onPress={() => {
                    const employeeId =
                      item.id === "all" ? "all" : item.id.toString();
        ...
        <View className="flex-row justify-between mt-2">
          ...
          >
            ...
            {employeeStats.reduce(
              (acc, curr) => acc + Number(curr.expense_count),
              0,
            )}
          </Text>
          <Text
          ...
          >
            ...
            {Math.round(
              (employeeStats.reduce(
                (acc, curr) => acc + Number(curr.approved_count),
                0,
              ) /
                employeeStats.reduce(
                  (acc, curr) => acc + Number(curr.expense_count),
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.EXPO_PUBLIC_API_URL}/pdf-reports/expense`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
