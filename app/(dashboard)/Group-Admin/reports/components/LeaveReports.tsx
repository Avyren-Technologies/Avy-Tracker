...
export default function LeaveReports({
  ...
  const fetchLeaveAnalytics = async () => {
    try {
      ...
      setError(
        error.response?.data?.error || "Failed to fetch leave analytics",
      );
    } finally {
      setLoading(false);
    }
  ...
  const renderMetricsCards = () => (
    <View className="flex-row flex-wrap justify-between mb-4">
      <View
        className={`w-[48%] p-4 rounded-lg mb-4 ${isDark ? "bg-gray-800" : "bg-white"}`}
      >
        <Text
    ...
    switch (graphType) {
      case "requests":
        return (
          <BarChart
            data={{
              labels: analytics.leaveTypes.map(
                (lt) => lt.leave_type.split(" ")[0],
              ),
              datasets: [
                {
      ...
      case "balances":
        return (
          <View>
            <BarChart
              data={{
                labels: analytics.balances.leave_types_balances.map(
                  (b) => b.leave_type.split(" ")[0],
                ),
                datasets: [
                  {
                    data: analytics.balances.leave_types_balances.map(
                      (b) => b.total_available,
                    ),
                  },
                ],
              ...
              >
                ...
                <View className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden">
                  <View
                    ...
                    style={{
                      width: `${Math.min((lt.total_days / lt.default_days) * 100, 100)}%`,
                    }}
                  />
                </View>
              </View>
