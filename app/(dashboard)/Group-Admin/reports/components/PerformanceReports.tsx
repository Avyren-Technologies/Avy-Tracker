  ...
  const renderGraph = () => {
    ...
    switch (graphType) {
          ...
          datasets: [
            {
              data: analytics.attendance
                .slice(0, 5)
                .map((a) => Number(a.punctuality_rate) || 0),
            },
          ],
        };
      ...
      case "expenses":
        ...
        const expenseData = sortedExpenses
          .map((e, index) => {
            const approvalRate =
              e.total_expenses > 0
            ...
            <View className="flex-row flex-wrap justify-between mt-2">
              {sortedExpenses.map((item, index) => (
                <View key={index} className="w-1/2 mb-2 flex-row items-center">
                  ...
                  >
                    {item.employee_name.split(" ")[0]}: {item.approved_expenses}
                    /{item.total_expenses} exp
                  </Text>
                </View>
              ))}
            </View>
