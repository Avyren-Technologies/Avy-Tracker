...
export default function GroupAdminLeaveInsights() {
  ...
  return (
    <View className="flex-1 p-6">
      ...
      {/* Tabs */}
      <View
        className={`flex-row rounded-lg mb-6 p-1 ${isDark ? "bg-gray-800" : "bg-gray-100"}`}
      >
        {tabs.map((tab) => (
          <TouchableOpacity
            key={tab.id}
            ...
            className={`flex-1 flex-row items-center justify-center p-3 space-x-2 rounded-lg ${
              activeTab === tab.id ? (isDark ? "bg-gray-700" : "bg-white") : ""
            }`}
          >
            <Ionicons
      ...
      <View className="flex-1">
        {activeTab === "requests" ? <LeaveRequests /> : <LeaveBalance />}
      </View>
    </View>
  );
}
