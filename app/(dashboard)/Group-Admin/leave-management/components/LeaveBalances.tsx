...
export default function LeaveBalances() {
      ...
      if (employeesResponse.data && employeesResponse.data.length > 0) {
        ...
        for (const employee of employeesResponse.data as EmployeeInfo[]) {
          try {
            ...
            const balancesWithUserInfo = response.data.map(
              (balance: BackendLeaveBalance) => {
                return {
                  ...balance,
                  user_id: employee.id,
            ...
            console.error(
              `Error fetching balances for employee ${employee.id}:`,
              error,
            );
          }
        }
  ...
  const employeeBalances = filteredBalances.reduce(
    (acc, balance) => {
  ...
  const renderEmployeeCard = (employeeData: any) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          setSelectedEmployee(isSelected ? null : employeeData.id.toString())
        }
      ...
      >
        ...
        {isSelected && (
          <View className="p-4">
            {employeeData.balances.map(
              (balance: LeaveBalance, index: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!isLast && (
                      <View
                        className={`h-[1px] mt-6 ${isDark ? "bg-gray-700" : "bg-gray-200"}`}
                      />
                    )}
                  </View>
                );
