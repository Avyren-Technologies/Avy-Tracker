...
export default function ExpenseDetailView() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      setExpense((prev) =>
        prev ? { ...prev, documents: response.data } : null,
      );
    } catch (error) {
    ...
    >
      ...
      <ScrollView className="flex-1 p-4">
        ...
        >
          ...
          <DetailRow
            label="Vehicle Type"
            value={expense.vehicle_type}
            isDark={isDark}
          />
          <DetailRow
            label="Vehicle Number"
            value={expense.vehicle_number}
            isDark={isDark}
          />
          <DetailRow
        ...
        {expense.documents && expense.documents.length > 0 && (
          ...
          >
            ...
            {expense.documents.map((doc) => (
              <DocumentViewer key={doc.id} document={doc} isDark={isDark} />
            ))}
          </View>
        )}
