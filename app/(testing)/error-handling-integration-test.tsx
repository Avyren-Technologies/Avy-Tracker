...
const ErrorHandlingIntegrationTest: React.FC = () => {
  ...
  const testErrorCreation = async () => {
    ...
    if (
      !(cameraError as any).userMessage ||
      !(cameraError as any).suggestions.length
    ) {
  ...
  const testErrorMapping = async () => {
    ...
    const mappedError =
      ErrorHandlingService.mapErrorToFaceVerificationError(networkError);
    ...
    const mappedTimeout =
      ErrorHandlingService.mapErrorToFaceVerificationError(timeoutError);
  ...
  const testFallbackMechanism = async () => {
    ...
    const shouldFallback = ErrorHandlingService.shouldUseFallback(
      criticalError,
      1,
    );
    if (!shouldFallback) {
    ...
    const shouldNotFallback = ErrorHandlingService.shouldUseFallback(
      retryableError,
      1,
    );
    if (shouldNotFallback) {
      throw new Error(
        "Should not use fallback for lighting error on first attempt",
      );
    }
  };
        ...
        onPress={() => {
          ...
          Alert.alert(
            ...
            [
              {
                text: "Clear History",
                onPress: () => ErrorHandlingService.clearErrorHistory(),
              },
              { text: "OK" },
            ],
          );
