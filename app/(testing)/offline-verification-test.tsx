...
export default function OfflineVerificationTest() {
  ...
  const runAllTests = async () => {
    ...
    try {
      ...
      await runTest('Connectivity Service Initialization', async () => {
        ...
        if (!state) throw new Error('Failed to initialize connectivity service');
      });
      ...
      await runTest('Offline Face Verification', async () => {
        const testUserId = 12345;
        const testEncoding = JSON.stringify(Array(128).fill(0).map(() => Math.random()));
      ...
      await runTest('Queue Verification Data', async () => {
        const testData = {
          userId: 12345,
          faceEncoding: JSON.stringify(Array(128).fill(0).map(() => Math.random())),
          timestamp: new Date(),
      ...
      await runTest('Storage Statistics', async () => {
        ...
        if (typeof stats.queuedItems !== 'number' || 
  ...
  const testSyncFunctionality = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Sync test failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      setCurrentTest('');
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.statusContainer}>
        ...
        <Text style={styles.statusText}>Syncing: {isSyncing ? '🔄' : '✅'}</Text>
        ...
        <Text style={styles.statusText}>Last Sync: {lastSync ? lastSync.toLocaleString() : 'Never'}</Text>
