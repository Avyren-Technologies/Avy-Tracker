...
export default function SettingsNavigationIntegrationTest() {
  ...
  const runTests = async () => {
    ...
    try {
      if (typeof navigateToFaceConfiguration === 'function' &&
        ...
        updateTestResult('deep-link-utils', 'passed', 'All utility functions imported successfully');
      } else {
        updateTestResult('deep-link-utils', 'failed', 'Some utility functions missing');
      }
    } catch (error) {
    ...
    try {
      ...
      const allFunctionsValid = functions.every(fn => typeof fn === 'function');
      
      if (allFunctionsValid) {
        updateTestResult('navigation-functions', 'passed', 'All navigation functions are callable');
      } else {
        updateTestResult('navigation-functions', 'failed', 'Some navigation functions are not callable');
      }
    } catch (error) {
      updateTestResult('navigation-functions', 'failed', `Function check error: ${error}`);
    }
    ...
    try {
      ...
      if (initialData.action === null && initialData.source === null && initialData.params === null) {
        updateTestResult('async-storage-integration', 'passed', 'AsyncStorage integration working');
      } else {
        updateTestResult('async-storage-integration', 'failed', 'AsyncStorage data not cleared properly');
      }
    } catch (error) {
      updateTestResult('async-storage-integration', 'failed', `AsyncStorage error: ${error}`);
    }
    ...
    try {
      ...
      if (result === true) {
        updateTestResult('face-setup-prompt', 'passed', 'Face setup prompt function works');
      } else {
        updateTestResult('face-setup-prompt', 'failed', 'Face setup prompt returned false');
      }
    } catch (error) {
      updateTestResult('face-setup-prompt', 'failed', `Prompt function error: ${error}`);
    }
    ...
    try {
      ...
      if (data.action === 'configure' && data.source === 'test' && data.params?.test === true) {
        updateTestResult('deep-link-data-management', 'passed', 'Deep link data stored and retrieved correctly');
      } else {
        updateTestResult('deep-link-data-management', 'failed', 'Deep link data not stored/retrieved correctly');
      }
      
      // Clean up
      await clearDeepLinkData();
    } catch (error) {
      updateTestResult('deep-link-data-management', 'failed', `Data management error: ${error}`);
    }
  ...
  const runManualTests = () => {
    Alert.alert(
      'Manual Tests',
      'Choose a manual test to run:',
      [
        {
          text: 'Test Face Registration Navigation',
          onPress: () => {
            Alert.alert('Test', 'This would navigate to Face Registration screen');
            // navigateToFaceRegistration();
          }
        },
        {
          ...
          onPress: () => {
            Alert.alert('Test', 'This would navigate to Face Configuration screen');
            // navigateToFaceConfigurationDirect();
          }
        },
        {
          ...
          onPress: () => {
            Alert.alert('Test', 'This would use smart navigation based on registration status');
            // navigateToFaceSetup();
          }
        },
        {
          ...
          onPress: async () => {
            Alert.alert('Test', 'This would set up deep link and navigate to settings');
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={[styles.section, { backgroundColor: isDark ? '#1f2937' : '#ffffff' }]}>
          ...
          {testResults.map((test) => (
            <View key={test.id} style={[styles.testItem, { borderColor: isDark ? '#374151' : '#e5e7eb' }]}>
              <View style={styles.testHeader}>
                ...
                />
                <Text style={[styles.testTitle, { color: isDark ? '#ffffff' : '#111827' }]}>
                  {test.title}
                </Text>
              </View>
              {test.message && (
                <Text style={[styles.testMessage, { color: isDark ? '#9ca3af' : '#6b7280' }]}>
                  {test.message}
                </Text>
              )}
            </View>
        ...
        <View style={[styles.section, { backgroundColor: isDark ? '#1f2937' : '#ffffff' }]}>
          ...
          <View style={styles.requirementItem}>
            ...
            <Text style={[styles.requirementText, { color: isDark ? '#ffffff' : '#111827' }]}>
              2.4: Face configuration settings access
            </Text>
          </View>
