...
export default function VerificationFlowTest() {
  ...
  const {
    ...
    onFlowCompleted: (summary) => {
      addTestResult(
        `Flow completed: ${summary.status} (${summary.confidenceScore}% confidence)`,
      );
    },
    onFlowFailed: (summary) => {
      addTestResult(
        `Flow failed: ${summary.status} (${summary.failedSteps.join(", ")} failed)`,
      );
    },
    ...
    onPerformanceMetrics: (metrics) => {
      addTestResult(
        `Performance: ${metrics.totalLatency}ms total, ${metrics.completedSteps}/${metrics.stepCount} steps`,
      );
    },
  });
  ...
  const mockLocationVerification = useCallback(
    async (shouldSucceed = true): Promise<LocationResult> => {
  ...
  const mockFaceVerification = useCallback(
    async (shouldSucceed = true): Promise<FaceVerificationResult> => {
  ...
  const testManagerOverride = useCallback(async () => {
    try {
      ...
      if (flowState && canOverride) {
        await applyOverride(
          "Testing manager override functionality",
          mockUserId,
        );
      } else {
  ...
  const handleOrchestratorSuccess = useCallback(
    (summary: VerificationFlowSummary) => {
        ...
        >
          ...
          <View style={styles.statusRow}>
            ...
            <Text
              style={[
                styles.value,
                { color: isInProgress ? "#f59e0b" : textColor },
              ]}
            >
              {isInProgress ? "Yes" : "No"}
            </Text>
          ...
          <View style={styles.statusRow}>
            ...
            <Text
              style={[
                styles.value,
                { color: canOverride ? "#f59e0b" : textColor },
              ]}
            >
              {canOverride ? "Yes" : "No"}
            </Text>
        ...
        {performanceMetrics && (
          ...
          >
            ...
            <View style={styles.statusRow}>
              ...
              <Text style={[styles.value, { color: textColor }]}>
                {performanceMetrics.avgStepLatency
                  ? `${Math.round(performanceMetrics.avgStepLatency)}ms`
                  : "N/A"}
              </Text>
            </View>
            <View style={styles.statusRow}>
            ...
            <View style={styles.statusRow}>
              ...
              <Text
                style={[
                  styles.value,
                  {
                    color: performanceMetrics.fallbackMode
                      ? "#f59e0b"
                      : textColor,
                  },
                ]}
