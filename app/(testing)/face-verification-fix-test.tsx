    ...
    try {
      ...
      if (statusResponse.ok) {
        ...
        if (!statusData.registered || !statusData.active) {
          addResult(
            "❌ No active face profile found. Please register a face first.",
          );
          return;
        }
      } else {
      ...
      if (verifyResponse.ok) {
        ...
        addResult(
          `📊 Verification result: success=${verifyData.success}, confidence=${verifyData.confidence}`,
        );
        ...
        if (verifyData.success) {
          ...
          addResult(
            `⚠️ Verification failed with confidence: ${verifyData.confidence}`,
          );
          addResult(
            "💡 This might be expected with mock data - try with real face data",
          );
        }
      } else {
        const verifyError = await verifyResponse.json();
        addResult(
          `❌ Verification failed: ${verifyError.error || verifyResponse.status}`,
        );
      }
      ...
      if (comparisonResponse.ok) {
        ...
        addResult(
          `✅ Encoding comparison result: ${comparisonData.similarity}`,
        );
      } else {
        addResult(
          "ℹ️ Encoding comparison endpoint not available (this is normal)",
        );
      }
    } catch (error: any) {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          This test verifies that the face verification fixes are working
          correctly
        </Text>
      </View>
        ...
        <View style={styles.resultsContainer}>
          ...
          {testResults.length === 0 ? (
            <Text style={styles.noResults}>
              No test results yet. Tap a test button to start.
            </Text>
          ) : (
