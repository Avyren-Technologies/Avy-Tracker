...
export default function BiometricStorageServiceTest() {
  ...
  const [faceEncoding, setFaceEncoding] = useState('sample_face_encoding_data_12345');
  ...
  const testKeyGeneration = async () => {
    try {
      ...
      const testData = { type: 'face' as const, data: { test: 'data' }, metadata: { createdAt: new Date().toISOString(), deviceInfo: {} } };
      
      await service.storeBiometricData('test-user-1', testData);
      await service.storeBiometricData('test-user-2', testData);
      
      addResult('✅ Key generation: Encryption keys working through storage operations');
  ...
  const testBasicStorage = async () => {
    try {
      ...
      addResult(exists ? '✅ Exists check: Found data' : '❌ Exists check: No data found');
      ...
      addResult(!existsAfterDelete ? '✅ Delete verification: Data removed' : '❌ Delete verification: Data still exists');
  ...
  const testCleanup = async () => {
    try {
      ...
      const stats = await service.getStorageStats();
      addResult(`📊 Post-cleanup stats: ${stats.totalBiometrics} biometrics, ${stats.storageSize} bytes`);
  ...
  const testCompleteDataDeletion = async () => {
    try {
      ...
      if (stats.totalBiometrics === 0) {
        ...
        addResult(`❌ Delete verification: ${stats.totalBiometrics} biometrics remain`);
      }
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 20, backgroundColor: '#f5f5f5' }}>
      ...
      <View style={{ marginBottom: 20 }}>
        <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 5 }}>User ID:</Text>
        <TextInput
          style={{
            borderWidth: 1,
            borderColor: '#ddd',
        ...
        <Text style={{ fontSize: 16, fontWeight: '600', marginBottom: 5 }}>Face Encoding:</Text>
        <TextInput
          style={{
            borderWidth: 1,
            borderColor: '#ddd',
      ...
      <View style={{ marginBottom: 20 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
          <TouchableOpacity
            ...
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Key Gen</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{ backgroundColor: '#FF9500', padding: 10, borderRadius: 6, flex: 1 }}
            onPress={testBasicStorage}
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Storage</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{ backgroundColor: '#AF52DE', padding: 10, borderRadius: 6, flex: 1 }}
            onPress={testStorageStatistics}
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Stats</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{ backgroundColor: '#FF3B30', padding: 10, borderRadius: 6, flex: 1 }}
            onPress={testQuotaEnforcement}
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Quota</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{ backgroundColor: '#32D74B', padding: 10, borderRadius: 6, flex: 1 }}
            onPress={testDataIntegrity}
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Integrity</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{ backgroundColor: '#007AFF', padding: 10, borderRadius: 6, flex: 1 }}
            onPress={testBackupRestore}
            disabled={isLoading}
          >
            <Text style={{ color: 'white', textAlign: 'center', fontSize: 12 }}>Backup</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        <Text style={{ color: 'white', textAlign: 'center' }}>Clear Results</Text>
      </TouchableOpacity>
