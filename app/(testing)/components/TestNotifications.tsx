...
export default function TestNotifications() {
  ...
  const [remoteMessage, setRemoteMessage] = useState(
    "This is a remote test via Expo API",
  );
  ...
  useEffect(() => {
    ...
    if (Platform.OS === "android") {
      Notifications.getNotificationChannelsAsync().then((channels) => {
        addLog(
          `Android channels: ${channels.map((c) => c.name).join(", ") || "None"}`,
        );
      });
    }
  }, []);
  ...
  const checkConnectivity = async () => {
    try {
      ...
      addLog(
        `Connectivity status: ${response.status === 200 ? "OK" : "Failed"}`,
      );
      ...
      <View className="px-4 py-6">
        ...
        {expoPushToken && (
          ...
          >
            ...
            <Text
              selectable
              className={`p-2 rounded ${isDark ? "bg-gray-700 text-blue-300" : "bg-gray-100 text-blue-800"}`}
            >
              {expoPushToken}
            </Text>
          </View>
