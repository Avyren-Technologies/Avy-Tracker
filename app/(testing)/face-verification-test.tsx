...
export default function FaceVerificationTest() {
  ...
  const handleSuccess = (result: FaceVerificationResult) => {
    ...
    setShowModal(false);
    Alert.alert('Success!', `Verification completed with ${Math.round(result.confidence * 100)}% confidence`);
  };
    ...
    <SafeAreaView style={{ flex: 1, padding: 20, backgroundColor: '#f5f5f5' }}>
      ...
      {lastResult && (
        <View style={{
          ...
          borderRadius: 8,
          borderLeftWidth: 4,
          borderLeftColor: lastResult.startsWith('Success') ? '#34C759' : '#FF3B30',
        }}>
