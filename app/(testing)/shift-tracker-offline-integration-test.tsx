...
export default function ShiftTrackerOfflineIntegrationTest() {
  ...
  const initializeServices = async () => {
    try {
      ...
      addResult('❌ Failed to initialize services: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };
  ...
  const simulateShiftStart = async () => {
    try {
      ...
      const mockFaceEncoding = JSON.stringify(Array(128).fill(0).map(() => Math.random()));
      ...
      if (simulatedOffline || !isOnline) {
        ...
        if (faceResult.success) {
          addResult(`✅ Face verification successful (confidence: ${faceResult.confidence.toFixed(2)})`);
      ...
      addResult('❌ Shift start failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };
  ...
  const simulateShiftEnd = async () => {
    try {
      ...
      addResult('❌ Shift end failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };
  ...
  const testOfflineCapabilities = async () => {
    try {
      ...
      const connectivity = await ConnectivityService.getCurrentState();
      addResult(`📶 Connection: ${connectivity.type}, Online: ${connectivity.isConnected && connectivity.isInternetReachable}`);
      
      addResult('🎉 Offline capability tests completed');
      
    } catch (error) {
      addResult('❌ Offline capability test failed: ' + (error instanceof Error ? error.message : 'Unknown error'));
    }
  };

  const toggleOfflineMode = () => {
    setSimulatedOffline(!simulatedOffline);
    addResult(`🔄 Switched to ${!simulatedOffline ? 'OFFLINE' : 'ONLINE'} simulation mode`);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        ...
        <TouchableOpacity style={styles.button} onPress={testOfflineCapabilities}>
          <Text style={styles.buttonText}>Test Offline</Text>
        </TouchableOpacity>

        <TouchableOpacity 
          style={[styles.button, simulatedOffline ? styles.onlineButton : styles.offlineButton]} 
