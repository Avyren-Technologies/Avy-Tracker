...
export default function FaceVerificationServiceTest() {
  ...
  const testFaceEncoding = async () => {
    try {
      ...
      addResult(
        `✅ Face encoding comparison: ${(similarity * 100).toFixed(1)}% similarity`,
      );
    } catch (error) {
      addResult(
        `❌ Face encoding test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const testFaceProfileStorage = async () => {
    try {
      ...
      addResult(
        `❌ Face profile storage test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const testFaceVerification = async () => {
    try {
      ...
      const result1 = await verifyFace(
        testUserId,
        mockFaceData,
        mockPhoto,
        true,
      ...
      addResult(
        `✅ Same face verification: ${result1.success ? "SUCCESS" : "FAILED"}`,
      );
      ...
      const result2 = await verifyFace(
        testUserId,
        differentFaceData,
        mockPhoto,
        false,
      ...
      addResult(
        `✅ Different face verification: ${result2.success ? "SUCCESS" : "FAILED"}`,
      );
      addResult(`   Confidence: ${(result2.confidence * 100).toFixed(1)}%`);
    } catch (error) {
      addResult(
        `❌ Face verification test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const testRegistrationStatus = async () => {
    try {
      ...
      addResult(
        `❌ Registration status test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const testCacheAndOffline = async () => {
    try {
      ...
      if (cache) {
        addResult(
          `✅ Found ${cache.verifications.length} cached verifications`,
        );
      } else {
      ...
      addResult(
        `✅ Found ${offline.length} offline verifications pending sync`,
      );
    } catch (error) {
      addResult(
        `❌ Cache and offline test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const testCleanup = async () => {
    try {
      ...
      const status = await getFaceRegistrationStatus(testUserId);
      addResult(
        `✅ Registration status after deletion: ${status.isRegistered ? "STILL REGISTERED" : "DELETED"}`,
      );
      ...
      addResult(
        `❌ Cleanup test failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
