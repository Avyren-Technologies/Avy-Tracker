...
export function TrackingProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleAppStateChange = useCallback(
    async (nextAppState: AppStateStatus) => {
      ...
      if (
        ...
        console.log(
          `App state changed: ${lastAppStateRef.current} -> ${nextAppState}`,
        );
          ...
          if (
            ...
            if (backgroundTrackingEnabled) {
              ...
              if (!isActive && trackingStatus === TrackingStatus.ACTIVE) {
                console.log(
                  "Background tracking should be active but is not, restarting...",
                );
  ...
  useEffect(() => {
    ...
    const initializeTracking = async () => {
      try {
        ...
        const storedTracking = await AsyncStorage.getItem(
          "backgroundTrackingEnabled",
        );
        const storedTrackingStatus =
          await AsyncStorage.getItem("trackingStatus");
        ...
        if (storedTracking === "true") {
          console.log(
            "Background tracking was previously enabled, restoring state",
          );
  ...
  useEffect(() => {
    const getDeviceInfo = async () => {
      try {
        ...
        console.log(
          `Device info: ${deviceManufacturer}, needs optimization: ${needsOptimization}`,
        );
      } catch (error) {
        ...
        if (status === "granted") {
          ...
          try {
            ...
            if (!(await Device.isDevice)) {
              console.log(
                "Running in iOS simulator - skipping significant location changes",
              );
              return;
            }
            ...
            if (!TaskManager.isTaskDefined("ios-significant-change")) {
              TaskManager.defineTask(
                ...
                async ({ data, error }) => {
                  if (error) {
                    console.error(
                      "iOS significant location changes error:",
                      error,
                    );
                    return;
                  }
                  ...
                  if (!isActive) {
                    console.log(
                      "Restarting background tracking from iOS significant change",
                    );

                    // Get stored config or use defaults
                    const configStr =
                      await AsyncStorage.getItem("trackingConfig");
            ...
            console.error(
              "Error setting up iOS significant location changes:",
              error,
            );
          }
        }
  ...
  const checkTrackingStatus = useCallback(async (): Promise<boolean> => {
    try {
      ...
      console.log(
        `Background permission status: ${bgStatus}, Foreground permission status: ${fgStatus}`,
      );
      ...
      const fullyActive =
        isActive && isTaskRegistered && locationServicesEnabled;

      if (!fullyActive) {
        console.warn(
          "Background tracking is only partially active or inactive",
        );
        ...
        if (!isTaskRegistered) {
          ...
          console.warn(
            "Task is registered but location updates are not active",
          );
  ...
  const configureAdaptiveTracking = useCallback(
    async (options: {
      battery: boolean;
      activity: boolean;
      stationary: boolean;
      ...
      try {
        ...
        console.log(
          `Adaptive tracking configured: battery=${options.battery}, activity=${options.activity}, stationary=${options.stationary}`,
        );
        return true;
      } catch (error) {
  ...
  const restartBackgroundTracking = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (isActive) {
        console.log(
          "Stopping current background tracking session before restart",
        );
      ...
      if (Platform.OS === "android") {
        if (deviceInfo.needsBatteryOptimization) {
          ...
          const hasAsked =
            await BatteryOptimizationHelper.hasAskedForBatteryOptimization();
      ...
      const { status: bgPermission } =
        await Location.getBackgroundPermissionsAsync();
      if (bgPermission !== "granted") {
        console.warn(
          "Background location permission not granted, requesting...",
        );
      ...
      if (Platform.OS === "ios") {
        const { status: fgPermission } =
          await Location.getForegroundPermissionsAsync();
        if (fgPermission !== "granted") {
          console.warn(
            "Foreground location permission not granted, requesting...",
          );
          const { status } = await Location.requestForegroundPermissionsAsync();
          if (status !== "granted") {
            console.error(
              "Foreground location permission denied after request",
            );
            return false;
          }
        }
      ...
      while (!success && attempts < maxAttempts) {
        ...
        try {
          ...
          if (success) {
            console.log(
              `Background tracking restarted successfully (attempt ${attempts})`,
            );
            ...
            if (!isRunning) {
              console.warn(
                "Background tracking appeared to start but is not active, retry",
              );
              success = false;
            }
          }
        ...
        if (!success && attempts < maxAttempts) {
          console.log(
            `Waiting before retry ${attempts + 1} of ${maxAttempts}...`,
          );
      ...
      if (success) {
        ...
        console.error(
          `Failed to restart background tracking after ${maxAttempts} attempts`,
        );
        return false;
      }
    } catch (error) {
