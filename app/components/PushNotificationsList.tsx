...
const PushNotificationsList = forwardRef(
  (
    ...
    const { decrementUnreadCount, incrementUnreadCount, setUnreadCount } =
      useNotifications();
    ...
    useEffect(() => {
      const loadInitialUnreadCount = async () => {
        try {
          ...
          const readNotifications = storedReadStatus
          ...
          if (cachedData) {
            ...
            const currentUnreadCount = cachedData.filter(
          ...
          console.error(
            "[Notifications] Error loading initial unread count:",
            error,
          );
        }
      };
      ...
      async (notifs: Notification[]) => {
        try {
          ...
          return notifs.map((notification) => ({
            ...notification,
            read:
              readNotifications[notification.uniqueId || notification.id] ||
              false,
          }));
        } catch (error) {
    ...
    const saveReadStatus = useCallback(
    ...
    const saveReadStatusBulk = useCallback(
    ...
    const fetchNotifications = useCallback(
      async (page = 0, append = false) => {
        try {
          ...
          // Update the cache
          notificationCacheRef.current.set(
            cacheKey,
          ...
          if (append) {
            setNotifications((prevNotifications) => {
              ...
              const existingIds = new Set(
                prevNotifications.map((n) => n.uniqueId || n.id.toString()),
              );
    ...
    const handleNotificationPress = async (notification: Notification) => {
      try {
        ...
        if (notification.data?.screen) {
          ...
          const normalizedScreen = screen.startsWith("/")
            ? screen
            : `/${screen}`;
        ...
        console.error(
          "[Notification] Error handling notification press:",
          error,
        );
        Alert.alert(
          "Error",
