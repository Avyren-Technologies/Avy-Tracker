...
const LocationAccuracySettings: React.FC<LocationAccuracySettingsProps> = ({
  onClose,
}) => {
  ...
  const handleSliderChange = (
    field: keyof AccuracyFilterSettings,
    value: number,
  ) => {
    setSettings((prev) => ({
      ...prev,
          ...
          <Pressable
            ...
            onTouchMove={(e) => {
              ...
              const parentWidth = parentElement?.measure
                ? parentElement.measure(
                    (x: number, y: number, width: number) => width,
                  )
                : 300;
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.description}>
          Filters out low-quality location readings in challenging environments
      ...
      <View style={styles.section}>
        ...
        <View style={styles.sliderContainer}>
          <View style={styles.sliderLabelRow}>
            ...
            <Text style={styles.sliderValue}>
              {Math.round(settings.maxAccuracyRadius)}m
            </Text>
          </View>
          <CustomSlider
            minimumValue={50}
            ...
            onValueChange={(value: number) =>
              handleSliderChange("maxAccuracyRadius", value)
            }
            disabled={!settings.enabled}
          />
        ...
        <View style={styles.sliderContainer}>
          <View style={styles.sliderLabelRow}>
            ...
            <Text style={styles.sliderValue}>
              {Math.round(settings.goodAccuracyThreshold)}m
            </Text>
          </View>
          <CustomSlider
            minimumValue={5}
            ...
            onValueChange={(value: number) =>
              handleSliderChange("goodAccuracyThreshold", value)
            }
            disabled={!settings.enabled}
          />
          <View style={styles.sliderRangeLabels}>
      ...
      <View style={styles.helpSection}>
        ...
        <Text style={styles.helpText}>
          <Text style={styles.bold}>• Maximum Accuracy Radius:</Text> The
          maximum uncertainty radius (in meters) for a location reading to be
          considered usable. Larger values accept more readings but may include
          less accurate positions.
        </Text>
        <Text style={styles.helpText}>
          <Text style={styles.bold}>• Good Accuracy Threshold:</Text> Readings
          with accuracy better than this value are considered high quality.
          Affects confidence scoring.
        </Text>
        <Text style={styles.helpText}>
          <Text style={styles.bold}>• Confidence Threshold:</Text> Minimum
          confidence score (0.0-1.0) required for a location reading to be
          accepted. Higher values mean stricter filtering.
        </Text>
        <Text style={styles.helpText}>
          <Text style={styles.bold}>• Location Smoothing:</Text> Reduces jitter
          by averaging recent positions, improving visual stability of your
          location on the map.
        </Text>
      </View>
    </ScrollView>
