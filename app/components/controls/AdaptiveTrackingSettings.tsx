...
const AdaptiveTrackingSettings: React.FC<AdaptiveTrackingSettingsProps> = ({
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const batteryEnabled = await AsyncStorage.getItem(
          BATTERY_ADAPTIVE_TRACKING_KEY,
        );
        const activityEnabled = await AsyncStorage.getItem(
          ACTIVITY_ADAPTIVE_TRACKING_KEY,
        );
        const stationaryEnabled = await AsyncStorage.getItem(
  ...
  const handleToggle = async (
    ...
    try {
      ...
      Alert.alert(
        "Error",
        `Failed to update ${type} tracking setting. Please try again.`,
      );
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor }]}>
      ...
      <Text style={[styles.description, { color: textColor }]}>
        Optimize battery usage and tracking performance with these smart
        settings.
      </Text>
      ...
      {deviceInfo.needsBatteryOptimization && (
        <View style={[styles.warningBox, { borderColor: "#f59e0b" }]}>
          ...
          <Text style={[styles.warningText, { color: textColor }]}>
            Your device ({deviceInfo.manufacturer}) is known to have aggressive
            battery optimization. Enabling these settings is highly recommended.
          </Text>
        </View>
      )}
      ...
      <View style={styles.recommendedSettings}>
        <TouchableOpacity
          style={[styles.recommendedButton, { backgroundColor: accentColor }]}
          onPress={() => {
        ...
        <Text style={[styles.recommendedDescription, { color: textColor }]}>
          For optimal battery life and tracking accuracy, we recommend enabling
