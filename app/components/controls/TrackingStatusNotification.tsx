...
const TrackingStatusNotification: React.FC<TrackingStatusNotificationProps> = ({
  ...
  const { trackingStatus, currentLocation, backgroundTrackingEnabled } =
    useLocationStore();
  ...
  useEffect(() => {
    ...
    const trackingStartedSubscription = EventEmitter.addListener(
    ...
    const trackingStoppedSubscription = EventEmitter.addListener(
    ...
    const trackingRestartFailedSubscription = EventEmitter.addListener(
      "trackingAutoRestartFailed",
      () => {
        if (!compact) {
