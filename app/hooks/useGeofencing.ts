...
export function useGeofencing({
  ...
  const isLocationInAnyGeofence = useCallback(
    (location: any): boolean => {
      ...
      return geofences.some((geofence) => {
        ...
        if (
          ...
          (geofence as any).name
        ) {
          console.warn(
            `Unable to get coordinates from geofence: ${(geofence as any).name}`,
          );
          return false;
        }
  ...
  const isLocationInGeofence = useCallback(
    (location: Location, geofenceId: number): boolean => {
  ...
  const createGeofenceWithConfirmation = useCallback(
    async (
      name: string,
      ...
      try {
        ...
        Alert.alert("Error", `Failed to create geofence: ${errorMsg}`, [
          { text: "OK" },
        ]);
  ...
  const updateGeofenceWithConfirmation = useCallback(
    async (
      id: number,
      ...
      try {
        ...
        Alert.alert("Error", `Failed to update geofence: ${errorMsg}`, [
          { text: "OK" },
        ]);
  ...
  const deleteGeofenceWithConfirmation = useCallback(
  ...
  const createCircularBoundary = useCallback(
    (
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        coordinates.push(
          calculateCoordinateAtRadius(center, radiusInMeters, angle),
        );
      }
