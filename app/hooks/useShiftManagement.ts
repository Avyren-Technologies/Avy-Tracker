...
export function useShiftManagement({
  ...
  const completeShiftStart = useCallback(
    async (location: AppLocation): Promise<void> => {
      // Send to socket
  ...
  const completeShiftEnd = useCallback(
    async (location: AppLocation): Promise<void> => {
      // Send to socket
      if (socket && isConnected) {
  ...
  // Get shift history
  const getShiftHistory = useCallback(
    async (startDate: string, endDate: string): Promise<ShiftLocation[]> => {
      setIsLoading(true);
      try {
        const response = await axios.get(
