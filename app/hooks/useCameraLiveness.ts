...
export function useCameraLiveness(
  ...
  const eyeStateHistoryRef = useRef<Array<{ left: number; right: number; timestamp: number }>>([]);
  ...
  const processLivenessDetection = useCallback((currentFaceData: FaceDetectionData) => {
    ...
    if (recentHistory.length >= 5) {
      ...
      for (let i = 2; i < recentHistory.length - 2; i++) {
        const prev = (recentHistory[i - 1].left + recentHistory[i - 1].right) / 2;
        const curr = (recentHistory[i].left + recentHistory[i].right) / 2;
        const next = (recentHistory[i + 1].left + recentHistory[i + 1].right) / 2;
        ...
        if (prev > config.eyeOpenThreshold && 
          ...
          const blinkDuration = recentHistory[i + 1].timestamp - recentHistory[i - 1].timestamp;
          ...
          if (blinkDuration >= config.minBlinkDuration && 
              blinkDuration <= config.maxBlinkDuration &&
              timeSinceLastBlink > 500) { // Prevent duplicate detection
            
            blinkDetectedInHistory = true;
            lastBlinkTimeRef.current = now;
            setBlinkCount(prev => {
              const newCount = prev + 1;
              console.log('👁️ ✅ BLINK DETECTED! Count:', newCount, 'Duration:', blinkDuration, 'ms');
              return newCount;
            });
            break;
          }
        ...
        if (!blinkDetectedInHistory && i >= 2 && i < recentHistory.length - 1) {
          const prev = (recentHistory[i - 1].left + recentHistory[i - 1].right) / 2;
          const curr = (recentHistory[i].left + recentHistory[i].right) / 2;
          const next = (recentHistory[i + 1].left + recentHistory[i + 1].right) / 2;
          ...
          if (dipAmount > 0.001 && // Any detectable movement
              avgBaseline > 0.99 && // High baseline (eyes open)
              now - lastBlinkTimeRef.current > 500) { // Prevent spam
        ...
        if (!blinkDetectedInHistory && i >= 4 && i < recentHistory.length - 1) {
          const last5 = recentHistory.slice(i - 4, i + 1);
          const eyeValues = last5.map(h => (h.left + h.right) / 2);
          const mean = eyeValues.reduce((a, b) => a + b, 0) / eyeValues.length;
          ...
          if (stdDev > 0.0005 && // Any variation in eye openness
              mean > 0.99 && // High average (eyes generally open)
              now - lastBlinkTimeRef.current > 800) { // Prevent spam
        ...
        if (!blinkDetectedInHistory && i >= 3 && i < recentHistory.length - 1) {
          ...
          if (dipAmount > 0.01 && recoveryAmount > 0.005 && // Much more sensitive
              baseline > 0.98 && recovery > 0.98 && // Started and ended with eyes open
              now - lastBlinkTimeRef.current > 1200) { // Longer gap for fallback detection
            ...
            setBlinkCount(prev => {
              const newCount = prev + 1;
              console.log('👁️ ✅ TRADITIONAL FALLBACK BLINK! Count:', newCount);
              return newCount;
            });
            break;
          }
    ...
    const eyeMovementVariance = calculateEyeMovementVariance(eyeStateHistoryRef.current);
    setEyeMovementScore(eyeMovementVariance);

    // Calculate overall liveness score
    const currentBlinkCount = blinkDetectedInHistory ? blinkCount + 1 : blinkCount;
  ...
  const calculateEyeMovementVariance = (history: Array<{ left: number; right: number; timestamp: number }>): number => {
  ...
  const processFaceData = useCallback((faceData: FaceDetectionData) => {
    ...
    if (!isLivenessActive || !faceData) {
      console.log('👁️ Skipping face data processing:', { isLivenessActive, hasFaceData: !!faceData });
      return;
    }
