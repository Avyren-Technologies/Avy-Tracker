...
export function useSocket({
  ...
  const setupConnection = useCallback(async () => {
    ...
    if (now - lastConnectionAttempt < CONNECTION_THROTTLE_TIME) {
      console.log(
        "[useSocket] Throttling connection attempt, too many requests",
      );
      return;
    }
    ...
    try {
      ...
      if (globalSocket) {
        console.log(
          "[useSocket] Closing existing socket connection before reconnect",
        );
      ...
      socketInstance.on("connect", () => {
        console.log(
          `[useSocket] Connected to socket server with ID: ${socketInstance.id}`,
        );
        if (isMountedRef.current) {
          setIsConnected(true);
          setConnectionError(null);
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    console.log(
      `[useSocket] Scheduling reconnect in ${backoffTime}ms (attempt ${connectionAttempts.current})`,
    );
    ...
    reconnectTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (networkState.isConnected && networkState.isInternetReachable) {
          console.log(
            "[useSocket] Network available, attempting to reconnect...",
          );
          setupConnection();
        } else {
          console.log(
            "[useSocket] Network unavailable, skipping reconnect attempt",
          );
          if (isMountedRef.current) {
  ...
  const processQueuedUpdates = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        console.warn(
          "[useSocket] No auth token available for processing queued updates",
        );
        return;
      }
  ...
  const getOptimalUpdateInterval = useCallback(
    async (batteryLevel: number, isCharging: boolean): Promise<number> => {
      try {
        ...
        console.error(
          "[useSocket] Error getting optimal update interval:",
          error,
        );
  ...
  useEffect(() => {
    ...
    const handleAppStateChange = async (nextAppState: AppStateStatus) => {
      ...
      if (
        ...
        console.log(
          "App returned to foreground, checking socket connection status",
        );
    ...
    const checkNetworkState = async () => {
      ...
      try {
        ...
        if (lastNetworkStateRef.current !== isNetworkAvailable) {
          ...
          console.log(
            `[useSocket] Network state changed: ${isNetworkAvailable ? "available" : "unavailable"}`,
          );
    ...
    return () => {
      ...
      if (socketRefCount <= 0) {
        ...
        if (socketRef.current) {
          ...
          if (onGeofenceTransition)
            socketRef.current.off("geofence:transition", onGeofenceTransition);
        }
      }
    };
  ...
  const emitLocation = useCallback(
    (location: any) => {
      if (!socketRef.current || !socketRef.current.connected) {
        console.warn(
          "[useSocket] Socket not connected, queueing location update",
        );
