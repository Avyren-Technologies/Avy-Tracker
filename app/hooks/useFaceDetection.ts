...
export function useFaceDetection(
  ...
  const validateCameraRef = useCallback((cameraRef: any): boolean => {
    try {
      ...
      if (
        typeof cameraRef.current.isNativeViewMounted === "function" &&
      ...
      if (
        cameraRef.current.props &&
        cameraRef.current.props.isActive === false
      ) {
  ...
  const refreshCameraReference = useCallback(() => {
    ...
    if (persistentCameraRef.current && validateCameraRef(persistentCameraRef)) {
      ...
      console.warn(
        "⚠️ Persistent ref test failed: Could not get the Camera's native view tag! Does the Camera View exist in the native view-tree?",
      );
  ...
  const initializeMLKit = useCallback(async (): Promise<boolean> => {
    try {
      ...
      console.log(
        "ML Kit detector available, checking methods:",
        Object.keys(mlKitDetector),
      );
      ...
      console.log(
        "ML Kit Face Detection initialized successfully with config:",
  ...
  const initializeCamera = useCallback(async (): Promise<boolean> => {
    try {
      ...
      console.log(
        "Initializing camera with performance mode:",
        performanceMode,
      );
  ...
  const validateFaceQuality = useCallback(
    (
      ...
      let sizeQuality = 0;
      if (faceRatio >= 0.03 && faceRatio <= 0.7) {
        // 3% to 70% of screen - more lenient
      ...
      const overall =
        sizeQuality * 0.4 + lightingQuality * 0.3 + angleQuality * 0.3;
  ...
  const convertMLKitFaceToFaceData = useCallback(
    (
      mlKitFace: any,
      frameWidth: number,
      frameHeight: number,
  ...
  const processFaceDetection = useCallback(
    (detectedFaces: any[], frameWidth: number, frameHeight: number) => {
      ...
      try {
        ...
        if (
          !mlKitFace.frame ||
          typeof mlKitFace.leftEyeOpenProbability === "undefined" ||
          typeof mlKitFace.rightEyeOpenProbability === "undefined"
        ) {
        ...
        const faceData = convertMLKitFaceToFaceData(
          mlKitFace,
          frameWidth,
          frameHeight,
        );
  ...
  const processPeriodicFaceDetection = useCallback(async () => {
    ...
    if (!isMountedRef.current) {
      console.log(
        "🛑 processPeriodicFaceDetection early exit - component unmounted:",
        {
          isMounted: isMountedRef.current,
    ...
    if (!detectionIntervalRef.current) {
      console.log(
        "🛑 processPeriodicFaceDetection early exit - no active interval:",
        {
    ...
    if (!mlKitDetector || !validCamera || !isMountedRef.current) {
      ...
      if (mlKitDetector && !validCamera) {
        console.log(
          "🔍 Camera ref missing - detection will retry when camera is connected",
        );
      }
    ...
    try {
      console.log(
        "✅ Starting periodic face detection with enhanced photo processing...",
      );
      ...
      try {
        ...
        console.log("Photo file verification:", {
          ...
          exists: fileExists,
          fileSize: fileExists
            ? await fs.stat(tempPhoto.path).then((stat: any) => stat.size)
            : "N/A",
        });
        ...
        try {
          ...
          console.log(
            "Photo file read successfully, content length:",
            fileContent.length,
          );

          // Check if the photo has valid JPEG header
          if (
            fileContent.startsWith("/9j/") ||
            fileContent.startsWith("iVBORw0KGgo")
          ) {
            console.log("✅ Photo has valid image format header");
          } else {
            console.warn(
              "⚠️ Photo format header not recognized, might cause ML Kit issues",
            );
          }
          ...
          if (fileContent.length < 5000) {
            console.warn(
              "⚠️ Photo file seems too small, might be corrupted or incomplete",
            );
          }
        } catch (readError) {
        ...
        console.log(
          "Photo processing failed, but continuing with ML Kit detection...",
        );
      }
      ...
      try {
        ...
        if (!result?.faces || result.faces.length === 0) {
          console.log(
            "⚠️ No faces detected in live camera photo - trying alternative approaches...",
          );

          // Try with different URI format
          const alternativeUri = tempPhoto.path.startsWith("/")
            ? tempPhoto.path
            : `/${tempPhoto.path}`;
          ...
          try {
            ...
            if (
              alternativeResult?.faces &&
              alternativeResult.faces.length > 0
            ) {
        ...
        if (result?.faces && result.faces.length > 0) {
          console.log(
            "🎉 SUCCESS: Face detection completed with",
            result.faces.length,
            "face(s)",
          );
        } else {
      ...
      if (result?.faces && result.faces.length > 0) {
        ...
        console.log(
          "🎉 Face detection completed successfully - stopping all timers",
        );
      } else {
      ...
      console.log(
        `⏱️ Face detection process completed in ${(endTime - startTime).toFixed(2)}ms`,
      );
    } catch (error: any) {
  ...
  const frameProcessor = useFrameProcessor((frame) => {
    ...
    try {
      ...
      if (frame.timestamp % 100 === 0) {
        // Log every 100 frames instead of every frame
        runOnJS(() => {
          console.log(
            "Frame processor active - would implement real-time detection here",
          );
        })();
      }
    } catch (error) {
  ...
  const detectFacesFromFrame = useCallback(async (): Promise<any> => {
    try {
      ...
      console.log(
        "Frame processor not fully implemented, using photo fallback",
      );
      return null;
    } catch (error) {
  ...
  const startDetection = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (detectionIntervalRef.current) {
        console.log(
          "🔄 Face detection already running (interval active), skipping start request",
        );
        return true;
      }
  ...
  const capturePhoto = useCallback(async (): Promise<CapturedPhoto> => {
    try {
      console.log("capturePhoto called with:", {
        ...
        hasTakePhotoMethod: cameraRef.current
      ...
      if (!cameraInstance) {
        throw new Error(
          "Camera not initialized - no valid camera instance available",
        );
      }
      ...
      if (!cameraInstance || typeof cameraInstance.takePhoto !== "function") {
        console.error("Camera component does not have takePhoto method:", {
          ...
          availableMethods: cameraInstance
            ? Object.keys(cameraInstance)
            : "no methods",
        });
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          console.log(
            `✅ Camera validation successful on attempt ${retryCount + 1}`,
          );
          break;
        } catch (validationError: any) {
          retryCount++;
          console.warn(
            `⚠️ Camera validation failed on attempt ${retryCount}:`,
      ...
      try {
        ...
        if (captureError.message.includes("native view tag")) {
          console.log(
            "🔄 Native view tag error detected - implementing advanced camera recovery...",
          );
          ...
          for (let attempt = 1; attempt <= 3; attempt++) {
            try {
              console.log(
                `🔄 Recovery attempt ${attempt}: Forcing camera reference refresh...`,
              );
              ...
              if (
                refreshedCamera &&
                typeof refreshedCamera.takePhoto === "function"
              ) {
                console.log(
                  `🔄 Recovery attempt ${attempt}: Testing camera responsiveness...`,
                );
                ...
                try {
                  ...
                  console.log(
                    `✅ Photo capture successful on recovery attempt ${attempt}:`,
                    photo,
                  );
                  ...
                  console.warn(
                    `⚠️ Recovery attempt ${attempt} test failed:`,
                    testError.message,
                  );
                ...
                console.warn(
                  `⚠️ Recovery attempt ${attempt}: Camera still invalid after refresh`,
                );
              }
            } catch (recoveryError: any) {
              console.warn(
                `⚠️ Recovery attempt ${attempt} failed:`,
          ...
          if (!recoverySuccess) {
            console.log(
              "🔄 All recovery attempts failed - trying camera reconnection...",
            );
            ...
            try {
              ...
              if (finalCamera && typeof finalCamera.takePhoto === "function") {
                try {
                  ...
                  console.log(
                    "✅ Photo capture successful after camera reconnection:",
                    photo,
                  );
                  recoverySuccess = true;
                } catch (finalError: any) {
                  console.error(
                    "❌ Final camera recovery attempt failed:",
                    finalError.message,
                  );
                }
              }
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState: AppStateStatus) => {
      ...
      console.log(
        `Face detection app state changed: ${appStateRef.current} -> ${nextAppState}`,
      );
  ...
  const refreshCameraRef = useCallback(async () => {
    ...
    if (
      persistentCameraRef.current &&
      typeof persistentCameraRef.current.takePhoto === "function"
    ) {
      console.log("🔍 Persistent camera ref is valid - attempting recovery...");
    ...
    console.log(
      "❌ Camera reference is still invalid after refresh - reset to detached state",
    );
    return false;
  }, []);
  ...
  const monitorCameraState = useCallback(async () => {
    ...
    if (!cameraRef.current && persistentCameraRef.current) {
      console.log(
        "🔍 Main ref is null but persistent ref exists - attempting recovery...",
      );
  ...
  const setCameraRef = useCallback((cameraComponent: any) => {
    ...
    if (cameraComponent) {
      ...
      console.log(
        ...
        typeof cameraComponent.takePhoto === "function",
      );
    ...
    console.log("✅ Camera reference set successfully:", {
      ...
      hasTakePhotoMethod: (cameraRef as any).current
        ? typeof (cameraRef as any).current.takePhoto === "function"
        : false,
    ...
    if (detectionIntervalRef.current && cameraComponent) {
      console.log(
        "🔄 Camera connected during active detection - detection will now proceed",
      );
    }
  }, []);
  ...
  const getCameraInstance = useCallback(() => {
    ...
    if (persistentCameraRef.current && validateCameraRef(persistentCameraRef)) {
      console.log(
        "🔄 getCameraInstance: main ref is null/invalid, using persistent ref",
      );
      ...
      console.log(
        "✅ getCameraInstance: restored camera instance from persistent ref",
      );
  ...
  const hasTakePhotoMethod = useCallback(() => {
    ...
    if (
      ...
      console.log(
        "🔄 hasTakePhotoMethod: main ref is null, but persistent ref has takePhoto method",
      );
      ...
      console.log(
        "✅ hasTakePhotoMethod: restored camera instance from persistent ref",
      );
      return true;
    }

    console.log(
      "❌ hasTakePhotoMethod: no camera with takePhoto method available",
    );
    return false;
  }, []);
