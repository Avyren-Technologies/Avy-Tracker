...
interface UseVerificationFlowReturn {
  ...
  // Actions
  startVerificationFlow: (shiftAction: 'start' | 'end', config?: Partial<VerificationConfig>) => Promise<void>;
  executeLocationVerification: (locationVerificationFn: () => Promise<LocationResult>) => Promise<void>;
  executeFaceVerification: (faceVerificationFn: () => Promise<FaceVerificationResult>) => Promise<void>;
  executeNextStep: (
...
export const useVerificationFlow = ({
  ...
  const [flowState, setFlowState] = useState<VerificationFlowState | null>(null);
  ...
  useEffect(() => {
    ...
    if (flowState.currentStepIndex !== lastStepIndexRef.current) {
      ...
      if (completedStepIndex >= 0 && completedStepIndex < flowState.steps.length) {
    ...
    if (flowState.overallStatus === 'in_progress' && !performanceTimerRef.current) {
    ...
    } else if (flowState.overallStatus !== 'in_progress' && performanceTimerRef.current) {
  ...
  }, [flowState, token, onStepCompleted, onFlowCompleted, onFlowFailed, onOverrideRequired, onPerformanceMetrics]);
  ...
  const startVerificationFlow = useCallback(async (
    ...
    try {
      const newFlowState = await initializeVerificationFlow(userId, shiftAction, config);
  ...
  const executeLocationVerification = useCallback(async (
    locationVerificationFn: () => Promise<LocationResult>
  ) => {
    if (!flowState) {
  ...
  const applyOverride = useCallback(async (reason: string, overrideBy: number) => {
    if (!flowState) {
      throw new Error('Verification flow not initialized');
    }

    try {
      const updatedFlowState = await applyManagerOverride(flowState, reason, overrideBy);
      setFlowState(updatedFlowState);
    } catch (error) {
  ...
  const getStepProgress = useCallback(() => {
    ...
    const completedSteps = flowState.steps.filter(step => step.completed).length;
  ...
  const getPerformanceMetrics = useCallback((): VerificationPerformanceMetrics | null => {
    ...
    const retryCount = flowState.steps.reduce((sum, s) => sum + s.retryCount, 0);

    const stepsWithLatency = flowState.steps.filter(s => s.startTime && s.endTime);
    const avgStepLatency = stepsWithLatency.length > 0 ?
      stepsWithLatency.reduce((sum, s) => sum + (s.endTime! - s.startTime!), 0) / stepsWithLatency.length :
      undefined;
    ...
    return {
      ...
      completedSteps,
      failedSteps: flowState.steps.filter(s => !s.completed && s.retryCount >= s.maxRetries).length,
      retryCount,
      ...
      maxStepLatency: stepsWithLatency.length > 0 ?
        Math.max(...stepsWithLatency.map(s => s.endTime! - s.startTime!)) : 0,
  ...
  // Computed values
  const isInProgress = flowState?.overallStatus === 'in_progress' || flowState?.overallStatus === 'pending';
  const currentStep = flowState && flowState.currentStepIndex < flowState.steps.length ?
