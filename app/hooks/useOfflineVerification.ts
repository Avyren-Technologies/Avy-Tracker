...
export const useOfflineVerification = () => {
  ...
  const initialize = useCallback(async () => {
    try {
      ...
      const connectivity =
        await OfflineVerificationService.getConnectivityStatus();
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to initialize offline verification";
   ...
   */
  const verifyFaceOffline = useCallback(
    async (
      userId: number,
      faceEncoding: string,
      ...
      try {
        ...
        const result = await OfflineVerificationService.verifyFaceOffline(
          userId,
          faceEncoding,
        );
        return result;
   ...
   */
  const queueVerificationData = useCallback(
    async (verificationData: {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Failed to queue verification data";
        setState((prev) => ({ ...prev, error: errorMessage }));
        throw error;
      }
    },
   ...
   */
  const cacheFaceProfile = useCallback(
    async (
      userId: number,
   ...
   */
  const validateLocationOffline = useCallback(
    async (location: {
      latitude: number;
      longitude: number;
    }): Promise<{
      ...
      try {
        ...
        return await OfflineVerificationService.validateLocationOffline(
          location,
        );
      } catch (error) {
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Offline location validation failed";
        setState((prev) => ({ ...prev, error: errorMessage }));
        throw error;
      }
    },
   ...
   */
  const cacheGeofences = useCallback(
    async (
      geofences: {
        id: string;
        name: string;
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Failed to cache geofences";
        setState((prev) => ({ ...prev, error: errorMessage }));
        throw error;
      }
    },
  ...
  const clearOfflineData = useCallback(async (): Promise<void> => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to clear offline data";
      setState((prev) => ({ ...prev, error: errorMessage }));
      throw error;
    }
  }, []);
